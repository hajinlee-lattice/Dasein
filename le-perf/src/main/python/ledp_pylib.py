...
def load(jetty_host, db_creds, table, key_cols, customer):
    os.system("java -cp '../../../target/dependency/*:../../../target/classes' com/latticeengines/perf/exposed/cli/ModelResourceCLI " 
              ...
              + " -c " + customer + " -t " + table + " -kc " + key_cols)
...
def submit_model(jetty_host, model_name, table, features, target, key_cols, algorithms, customer):
    alg_props = ""
    for algorithm in algorithms:
        alg_props += " --alg -n " + algorithm.name + " -vc " + str(algorithm.virtual_cores) + " -m " + str(algorithm.memory) + " -p " + str(algorithm.priority)
    os.system("java -cp '../../../target/dependency/*:../../../target/classes' com/latticeengines/perf/exposed/cli/ModelResourceCLI " 
              + "ledp submitmodel " + model_name + " " + jetty_host + " -c " + customer + " -t " + table + " -f " + features + " -T " + target 
              + " -kc " + key_cols + alg_props)
