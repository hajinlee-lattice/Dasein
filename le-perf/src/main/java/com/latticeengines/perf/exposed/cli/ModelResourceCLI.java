...
public class ModelResourceCLI {
	...
	private static void submitModel(String modelName) throws Exception {
		...
		String customer = optionMap.get("c"), table = optionMap.get("t"), features = optionMap.get("f"), targets = optionMap
				.get("T");
		modelDef.setName("Model Definition");
		List<Algorithm> algorithms = new ArrayList<Algorithm>();
		for (List<String> opList : algList) {
			String name = opList.get(0), virtualCores = opList.get(1), memory = opList.get(2), priority = opList.get(3);
	...
	private static void configAlgorithm(AlgorithmBase alg, String virtualCores, String memory, String priority) {
		...
		alg.setAlgorithmProperties(new StringBuilder().append("VIRTUALCORES=").append(virtualCores).append(" Memory=")
				.append(memory).append(" PRIORITY").append(priority).toString());
	}

	private static void createSamples() throws Exception {
		SamplingConfiguration samplingConfig = new SamplingConfiguration();
		String customer = optionMap.get("c"), table = optionMap.get("t"), trainingPercentage = optionMap.get("tp"), numOfSamples = optionMap
				.get("N");
	...
	private static void loadData() throws Exception {
		...
		String host = optionMap.get("H"), port = optionMap.get("P"), db = optionMap.get("db"), user = optionMap
				.get("u"), passwd = optionMap.get("ps"), customer = optionMap.get("c"), table = optionMap.get("t"), keyCol = optionMap
				.get("kc");
