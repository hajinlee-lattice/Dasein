...
public class CassandraGenericDaoImpl implements CassandraGenericDao {
    ...
    public void createTable(Table table) {
        ...
        CreateTableSpecification spec = CreateTableSpecification.createTable(table.getTableName());
        for (Column column : table.getPartitionColumns()) {
            spec.partitionKeyColumn(column.getColumnName(), ColumnTypeMapper.toCQLType(column.getColumnType()));
        }
        for (Column column : table.getClusterColumns()) {
            spec.clusteredKeyColumn(column.getColumnName(), ColumnTypeMapper.toCQLType(column.getColumnType()));
        }
        for (Column column : table.getNormalColumns()) {
            spec.column(column.getColumnName(), ColumnTypeMapper.toCQLType(column.getColumnType()));
        }
    ...
    public void dropColumn(String table, String column) {
        ...
        AlterTableSpecification tableSpec = AlterTableSpecification.alterTable(table);
        AlterTableCqlGenerator tableGenerator = new AlterTableCqlGenerator(tableSpec);

        DropColumnSpecification columnSpec = DropColumnSpecification.dropColumn(column);
        DropColumnCqlGenerator columnGenerator = new DropColumnCqlGenerator(columnSpec);
        StringBuilder sql = columnGenerator.toCql(tableGenerator.toCql(new StringBuilder()));
    ...
    public void createIndex(String table, String column, String index) {
        CreateIndexSpecification spec = CreateIndexSpecification.createIndex(index);
    ...
    @Override
    public void update(String table, String column, Object value, String whereColumn, Object whereColumnValue) {
    ...
    @Override
    public Map<String, Object> findByKey(String table, String whereColumn, String value) {
    ...
    @Override
    public Map<String, Object> findByKey2(String table, String whereColumn, String value) {
        String cql = "SELECT * FROM " + table + " WHERE " + whereColumn + " = \'" + value + "\'";
    ...
    @Override
    public List<Map<String, Object>> queryForListOfMapByTableColumn(String table, String column, Object value, int limit) {
