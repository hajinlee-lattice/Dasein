...
CREATE PROCEDURE `UpdateDataFeedTables`()
  BEGIN
    ...
    CREATE TABLE `DL_TENANT_MAPPING` (
      `PID`           BIGINT       NOT NULL AUTO_INCREMENT UNIQUE,
      `DL_LOAD_GROUP` VARCHAR(255),
      `DL_TENANT_ID`  VARCHAR(255),
      `TENANT_ID`     VARCHAR(255) NOT NULL,
      PRIMARY KEY (`PID`),
      UNIQUE (`DL_TENANT_ID`, `DL_LOAD_GROUP`)
    )
      ENGINE=InnoDB;
    ...
    CREATE INDEX IX_UNIQUE_ID
      ON `DATAFEED_TASK` (`UNIQUE_ID`);
