...
CREATE PROCEDURE `UpdateSchema`()
  BEGIN
	...
	alter table `PLAY` add index FK25833436865BC (FK_TENANT_ID), add constraint FK25833436865BC foreign key (FK_TENANT_ID) references `TENANT` (`TENANT_PID`) on delete cascade;
        ...
        create table `DATAFEED_EXECUTION` (`PID` bigint not null auto_increment unique, `EXECUTION` bigint not null, `STATUS` varchar(255) not null, FK_FEED_ID bigint not null, primary key (`PID`), unique (`EXECUTION`));
        ...
        create index IX_FEED_EXECUTION on `DATAFEED_EXECUTION` (`EXECUTION`);
        ...
        alter table `DATAFEED_IMPORT` add index FKADAC237C47E0C425 (FK_FEED_EXEC_ID), add constraint FKADAC237C47E0C425 foreign key (FK_FEED_EXEC_ID) references `DATAFEED_EXECUTION` (`PID`);
        ...
        alter table `DATAFEED_TASK` add index FK7679F31C679FF377 (FK_FEED_ID), add constraint FK7679F31C679FF377 foreign key (FK_FEED_ID) references `DATAFEED` (`PID`);
