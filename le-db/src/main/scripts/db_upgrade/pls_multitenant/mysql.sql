...
CREATE PROCEDURE `UpdateSchema`()
  BEGIN
	...
	create table `PLAY` (`PID` bigint not null auto_increment unique, `DESCRIPTION` varchar(255), `DISPLAY_NAME` varchar(255) not null, `IS_VISIBLE` boolean not null, `NAME` varchar(255) not null, `SEGMENT_NAME` varchar(255), FK_CALL_PREP_ID bigint, primary key (`PID`)) ENGINE=InnoDB;
