...
CREATE PROCEDURE `UpdatePLSTables`()
  BEGIN
    ...
    ALTER TABLE `PLAY_LAUNCH_CHANNEL` ADD COLUMN `DELETED` BIT NOT NULL DEFAULT 0;
  ...
  ALTER TABLE `PLS_MultiTenant`.`DATAFEED`
    ...
    CREATE TABLE `MIGRATION_TRACK` (
      ...
      `FK_TRACKING_REPORT` bigint(20) DEFAULT NULL COMMENT 'MIGRATE_TRACKING.pid',
    ...
    ALTER TABLE `PLS_MultiTenant`.`ATLAS_EXPORT` ADD COLUMN `CREATED_BY` VARCHAR(255);
    ALTER TABLE `PLS_MultiTenant`.`ATLAS_EXPORT` ADD COLUMN `ACCOUNT_RESTRICTION` LONGTEXT;
    ALTER TABLE `PLS_MultiTenant`.`ATLAS_EXPORT` ADD COLUMN `CONTACT_RESTRICTION` LONGTEXT;
    ALTER TABLE `PLS_MultiTenant`.`ATLAS_EXPORT` ADD COLUMN `PATH` VARCHAR(2048);
    ALTER TABLE `PLS_MultiTenant`.`ATLAS_EXPORT` ADD COLUMN `SCHEDULED` BIT NOT NULL DEFAULT 1;
    ALTER TABLE `PLS_MultiTenant`.`ATLAS_EXPORT` ADD COLUMN `CLEANUP_BY` DATETIME NOT NULL DEFAULT '1970-01-01 00:00:00';
    ALTER TABLE `PLS_MultiTenant`.`ATLAS_EXPORT` ADD COLUMN `STATUS` VARCHAR(255) NOT NULL DEFAULT 'COMPLETED';
    ALTER TABLE `PLS_MultiTenant`.`ATLAS_EXPORT` ADD COLUMN `FK_SEGMENT_ID` BIGINT;
    ALTER TABLE `PLS_MultiTenant`.`ATLAS_EXPORT` ADD CONSTRAINT `FK_ATLASEXPORT_FKSEGMENTID_METADATASEGMENT`
   	  FOREIGN KEY (`FK_SEGMENT_ID`) REFERENCES `PLS_MultiTenant`.`METADATA_SEGMENT` (`PID`) ON DELETE CASCADE;
  END;
