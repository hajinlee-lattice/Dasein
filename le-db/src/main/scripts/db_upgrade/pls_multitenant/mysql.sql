...
CREATE PROCEDURE `UpdatePLSTables`()
  ...
  ALTER TABLE `PLS_MultiTenant`.`PLAY_LAUNCH` ADD COLUMN `LAUNCH_UNSCORED` BIT NOT NULL DEFAULT 0;
  ...
  ALTER TABLE `PLS_MultiTenant`.`DATAFEED_TASK` ADD COLUMN  `SUBTYPE` varchar(255)

  ALTER TABLE `PLS_MultiTenant`.`DATAFEED_TASK` DROP INDEX `SOURCE` ,
  ADD UNIQUE INDEX `SOURCE` (`SOURCE` ASC, `FEED_TYPE` ASC, `FK_FEED_ID` ASC);

  ALTER TABLE `PLS_MultiTenant`.`MODEL_SUMMARY` ADD COLUMN `AVERAGE_REVENUE` DOUBLE PRECISION NULL;
  ...
  ALTER TABLE `PLS_MultiTenant`.`METADATA_SEGMENT` ADD COLUMN `DELETED` bit(1) DEFAULT 0;
