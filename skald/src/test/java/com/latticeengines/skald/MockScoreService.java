...
public class MockScoreService {
    ...
    public MockScoreService(final Map<String,String> modelsPMML, final List<CombinationElement> combination) {
        ...
        // When asking for the specified model combination, return the one from the definition
        when(combinationRetriever.getCombination(Mockito.any(CustomerSpace.class), Mockito.anyString()))
            .thenReturn(combination);
        
        // When asking for a ModelEvaluator, just look up the model in the definition and return
        // a new ModelEvaluator that operates against it.
        when(modelRetriever.getEvaluator(Mockito.any(CustomerSpace.class), Mockito.anyString(), Mockito.any(ScoreDerivation.class)))
            .thenAnswer(
                new Answer<ModelEvaluator>() {
                    ...
                    public ModelEvaluator answer(InvocationOnMock invocation) throws Throwable {
                        ...
                        ScoreDerivation derivation = (ScoreDerivation)args[2];
                        // look up the model
                        ...
                        return new ModelEvaluator(new StringReader(pmml), derivation);
                    }
        });
    }
