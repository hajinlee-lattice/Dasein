...
/**
 * This class and {@link RedshiftScheme} manage the ability to read and write
 * data to Amazon's Redshift via EMR. Because Redshift data is loaded into
 * Redshift via S3 but read from it via JDBC both these classes wrap the pairing
 * of an HFS {@link Tap} (for writing) and a JDBC {@link Tap} for reading behind
 * one {@link cascading.scheme.Scheme} object.
 */
public class RedshiftTap extends JDBCTap {
    ...
    /**
     * Redshift tap to stage data to S3 and then issue a JDBC COPY command to
     * specified Redshift table
     *
     * @param sinkMode
     *            use {@link SinkMode#REPLACE} to drop Redshift table before
     *            loading; {@link SinkMode#UPDATE} to not drop table for
     *            incremental loading
     */
    ...
    public RedshiftTap(String connectionUrl, String username, String password,
        ...
        String workingDirPath = hfsStagingDir + "/" + UUID.randomUUID();
        this.s3WorkingDir = workingDirPath.replaceAll("s3n://", "s3://");
        this.hfsStagingDir = new Hfs(scheme, workingDirPath);
    }
    ...
    public void sinkConfInit(FlowProcess<? extends Configuration> process, Configuration conf) {
        if (!useDirectInsert) {
            // if we haven't set the credentials beforehand try to set them from
            // the job conf
            ...
            // make the credentials to be used available to the JobConf if they
    ...
    protected String buildAuthenticationOptions() {
        return String.format(" CREDENTIALS 'aws_access_key_id=%s;aws_secret_access_key=%s' ",
                awsCredentials.getAwsAccessKey(), awsCredentials.getAwsSecretKey());
    }
    ...
    private String buildCopyOptions() {
        ...
        for (Map.Entry<RedshiftFactory.CopyOption, String> copyOption : redshiftScheme.getCopyOptions().entrySet()) {
            ...
            if (copyOption.getValue() == null)
                ...
                builder.append(copyOption.getKey().toString()).append(" ")
                        .append(copyOption.getKey().getArguments(copyOption.getValue()));
        }
