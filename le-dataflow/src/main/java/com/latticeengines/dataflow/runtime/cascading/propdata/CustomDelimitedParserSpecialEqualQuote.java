...
public class CustomDelimitedParserSpecialEqualQuote extends DelimitedParser {
    private static final Logger log = LoggerFactory
            .getLogger(CustomDelimitedParserSpecialEqualQuote.class);
    ...
    public CustomDelimitedParserSpecialEqualQuote(CsvToAvroFieldMapping fieldMap, String delimiter,
            String quote, boolean strict, boolean safe, FieldTypeResolver fieldTypeResolver) {
    ...
    static String preProcessValue(String value) {
        ...
        try {
            if (value.contains(EQUAL_QUOTE)
                    && (value.indexOf(EQUAL_QUOTE) != value.lastIndexOf(EQUAL_QUOTE))) {
                log.debug(
                ...
                for (int i = 0; i < value.length(); i++) {
                    ...
                    if (c == QUOTE.charAt(0)) {
                        ...
                        if (quoteCount % 2 == 0) {
                            basicSplits.add(runningSplit.substring(0,
                                    runningSplit.length() - COMMA.length()));
                            runningSplit = "";
                        }
                    }
                }
            ...
            log.error("Could not preprocess line, reverting to original line: " + ex.getMessage(),
                    ex);
