...
public class CoGroupBufferClosure extends JoinerClosure {
    ...
    public CoGroupBufferClosure(FlowProcess flowProcess, int numSelfJoins, Fields[] joinFields,
            Fields[] valueFields) {
        ...
        this.tupleCollectionFactory = loader.loadFactoryFrom(flowProcess, TUPLE_COLLECTION_FACTORY,
                SpillingTupleCollectionFactory.class);
    ...
    private void initLists() {
        ...
        if (numSelfJoins != 0) {
            ...
            collections[0] = new FalseCollection(); // we iterate this only once
                                                    // per grouping
    ...
    private Collection<Tuple> createTupleCollection(Fields joinField) {
        ...
        if (collection instanceof Spillable) {
            ((Spillable) collection)
                    .setSpillListener(new SpillListener(flowProcess, joinField, this.getClass()));
        }
    ...
    private Iterator<Tuple> makeIterator(final int pos, final Iterator values) {
        return new Iterator<Tuple>() {
            final int cleanPos = valueFields.length == 1 ? 0 : pos; // support
                                                                    // repeated
                                                                    // pipes
            ...
            private cascading.tuple.util.TupleBuilder makeBuilder(final Fields valueField,
                    final Fields joinField) {
    ...
    public void reset(Iterator<Tuple3<Tuple, Integer, Tuple>>... values) {
        ...
        if (collections[0] instanceof FalseCollection) { // force reset on
                                                         // FalseCollection
        ...
        while (values[0].hasNext()) {
            ...
            // we are guaranteed all the remainder tuples in the iterator are
            // from pos == 0
            if (numSelfJoins == 0 && pos == 0) {
                ((FalseCollection) collections[0])
                        .reset(createIterator(current, new FlinkUnwrappingIterator<>(values[0])));
                break;
            }

            collections[pos].add(current); // get the value tuple for this
                                           // cogroup
        }
    }
