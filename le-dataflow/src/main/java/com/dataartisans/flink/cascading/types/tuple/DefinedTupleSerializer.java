...
public class DefinedTupleSerializer extends TypeSerializer<Tuple> {
    ...
    public DefinedTupleSerializer(Fields fields, TypeSerializer[] fieldSers) {
        ...
        if (fieldSers == null || fieldSers.length != fields.size()) {
            throw new RuntimeException(
                    "Exactly one field serializer required for each tuple field.");
        }
    ...
    public Tuple copy(Tuple from) {
        ...
        for (int i = 0; i < from.size(); i++) {
            try {
                ...
                throw new FlowException("Unexpected type of field \"" + fields.get(i)
                        + "\" encountered. " + "Should have been " + fields.getType(i) + " but was "
                        + from.getObject(i).getClass() + ".", cce);
            }
        }
        return target;
    }
        ...
        for (int i = 0; i < value.size(); i++) {
            ...
            if (o != null) {
                try {
                    ...
                    throw new FlowException("Unexpected type of field \"" + fields.get(i)
                            + "\" encountered. " + "Should have been " + fields.getType(i)
                            + " but was " + o.getClass() + ".", cce);
                }
            }
        }
    }
    ...
    public boolean equals(Object obj) {
        if (obj instanceof DefinedTupleSerializer) {
            ...
            return other.canEqual(this) && fields.equals(other.fields)
