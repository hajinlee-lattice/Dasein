...
public class UnknownTupleComparator extends CompositeTypeComparator<Tuple> {
    ...
    public UnknownTupleComparator(int[] keyPositions, TypeComparator<?>[] comparators,
    ...
    public int compare(Tuple first, Tuple second) {
        ...
        for (int i = 0; i < keyPositions.length; i++) {
            int cmp = comparators[i].compare(first.getObject(keyPositions[i]),
                    second.getObject(keyPositions[i]));
    ...
    public void putNormalizedKey(Tuple value, MemorySegment target, int offset, int numBytes) {
        ...
        for (; i < this.numLeadingNormalizableKeys && numBytes > 0; i++) {
            ...
            this.comparators[i].putNormalizedKey(value.getObject(keyPositions[i]), target, offset,
                    len);
            numBytes -= len;
            offset += len;
        }
