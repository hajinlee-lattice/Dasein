...
public class FieldTypeInfo extends TypeInformation<Comparable> implements AtomicType {
            ...
            if (this.fieldTypeInfo != null && newFieldTypeInfo != null) {
                ...
                if (!this.fieldTypeInfo.equals(newFieldTypeInfo)) {
                    throw new RuntimeException(
                            "Cannot overwrite a field type by a new field types");
                }
    ...
    @Override
    public TypeComparator<Comparable> createComparator(boolean sortOrderAscending,
            ExecutionConfig config) {

        if (this.fieldTypeInfo != null) {
            TypeComparator<Comparable> fieldComparator = ((AtomicType) fieldTypeInfo)
                    .createComparator(sortOrderAscending, config);
            return new WrappingFieldComparator(fieldComparator, sortOrderAscending,
                    Comparable.class);
        } else {
            ...
            if (this.fieldComparator == null) {
                ...
                return new CustomFieldComparator(sortOrderAscending, this.fieldComparator,
                        this.createSerializer(config));
            }
        }
    }
