...
public class CoGroupReducer extends RichGroupReduceFunction<Tuple2<Tuple, Tuple[]>, Tuple> {
        ...
        try {
            ...
            if (sources.size() != 1) {
                throw new RuntimeException(
                        "FlowNode for CoGroupReducer may only have a single source");
            }
            ...
            throw new FlowException("internal error during CoGroupReducer configuration",
                    throwable);
        }
    ...
    @Override
    public void reduce(Iterable<Tuple2<Tuple, Tuple[]>> input, Collector<Tuple> output)
            throws Exception {
