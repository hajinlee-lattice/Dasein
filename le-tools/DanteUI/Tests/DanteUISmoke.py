...
class LeadInSFDCTest(unittest.TestCase):
    ...
    def testcase_1_LeadDetailPage(self):
        Logger.setInfoLog( '----test  case for Lead Detail page----')
        ...
        assert str(Lead_infos_page[0])==str(lead_dic["Title"]), Logger.setErrorLog('Display Name in Lead Dante Page is not right, actually is: "'+str(Lead_infos_page[0])+'" ,but expected should be: "'+str(lead_dic["Title"])+'" ')
        ...
        assert str(Lead_infos_page[1])==str(lead_dic["Score"]),  Logger.setErrorLog('Score in page is: "'+str(Lead_infos_page[1])+'", but should be: "'+str(lead_dic["Score"])+'"')
        assert str(Lead_infos_page[2])=="LATTICE ATTRIBUTES",  Logger.setErrorLog('Score Title in Page: "'+str(Lead_infos_page[2])+'" is not equal: "LATTICE ATTRIBUTES"')
        assert str(Lead_infos_page[3])=="INTERNAL ATTRIBUTES",  Logger.setErrorLog('Score Title in Page: "'+str(Lead_infos_page[3])+'" is not equal: "INTERNAL ATTRIBUTES"')

    def testcase_2_LeadListPage(self):
        Logger.setInfoLog( '----test  case for Lead list page----')
        page_Lead_Detail=DantePageHelper()
        danteData_DB=DanteDataHelper()
        page_Lead_Detail.LogInSaleForcePage()
    ...
    def testcase_3_ContactListPage(self):
        Logger.setInfoLog( '----test  case for Contact list page----')
    ...
    def testcase_4_ContactDetailPage(self):
        Logger.setInfoLog( '----test  case for Contact Detail page----')
    ...
    def testcase_5_LeadDanteWithoutScore(self):
        Logger.setInfoLog( '----test  case for Lead list page without score be configured----')
        ...
        assert str(lead_infos_page[0])==str(lead_dic["Title"]), Logger.setErrorLog('Display Name in Lead Dante Page is not right, actually is: "'+str(lead_infos_page[0])+'" ,but expected should be: "'+str(lead_dic["Title"])+'" ')
        #assert str(lead_infos_page[1])=="Lead Scoring Model", 'Score Title in Page: "'+str(lead_infos_page[1])+'" is not equal: "Lead Scoring Model"'
        assert str(lead_infos_page[1])=='',  Logger.setErrorLog('Score in page is: "'+str(lead_infos_page[1])+'", but should be: ""')

    def testcase_6_LeadDanteWithRate(self):
        Logger.setInfoLog( '----test  case for Lead list page without Rate be configured----')
        ...
        assert str(lead_infos_page[0])==str(lead_dic["Title"]), Logger.setErrorLog('Display Name in Lead Dante Page is not right, actually is: "'+str(lead_infos_page[0])+'" ,but expected should be: "'+str(lead_dic["Title"])+'" ')
        #assert str(lead_infos_page[1])=="Lead Scoring Model", 'Score Title in Page: "'+str(lead_infos_page[1])+'" is not equal: "Lead Scoring Model"'
        assert str(lead_infos_page[1])==lead_dic["Rate"],  Logger.setErrorLog('Score in page is: "'+str(lead_infos_page[1])+'", but should be: "'+str(lead_dic["Rate"])+'"')

class AccountInSFDCTest(unittest.TestCase):

    def testcase_1_AccountListPage(self):
        Logger.setInfoLog( '----test case for account list page----')
        ...
        assert play_count>0, Logger.setErrorLog('No play be displayed in Dante UI')
        for play_index in range (1,int(play_count)+1):
            play_TPs=playlist[int(play_index)-1]["TalkingPoints"]
            TPs_count=len(play_TPs)
            assert TPs_count>0, Logger.setErrorLog('No Talking points attribute be displayed in Dante UI')
    ...
    def testcase_2_AccountDetailPage(self):
        Logger.setInfoLog( '----test case for account detail page----')
