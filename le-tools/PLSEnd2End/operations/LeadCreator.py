...
def recordNewAdded(sequence, marketting_app, sobjects, lead_id, email):
    ...
    query = "INSERT INTO [Results]([sequence],[maps],[s_objects],[id],[email]) VALUES(%d,'%s','%s','%s','%s')" % (
        sequence, marketting_app, sobjects, lead_id, email);
...
def recordResultSet(id, email, operation, modified_date, scored, result):
    dlc = SessionRunner()
    connection_string = PLSEnvironments.SQL_BasicDataForIntegrationTest;
    query = "update results set operation='%s', executed_date=getdate(),modifiedDate='%s',scored=%s,result=%d where id='%s' and email='%s'" % (
        operation, modified_date[0:19], scored, result, id, email);
...
def verifyResult(operation, records):
    ...
    for r in records[1:]:
        ...
        recordResultSet(r["id"], r["email"], operation, r["latticeforleads__Last_Score_Date__c"],
                        r["latticeforleads__Score__c"], passed);

    return results;


class EloquaRequest():
    def __init__(self, base_url=PLSEnvironments.pls_ELQ_url, company=PLSEnvironments.pls_ELQ_company,
                 user=PLSEnvironments.pls_ELQ_user, password=PLSEnvironments.pls_ELQ_pwd):
            ...
            if response:
                if response.status_code == 201:
                    ...
                    if (contact_id != None):
                        ...
                        recordNewAdded(seq, PLSEnvironments.pls_marketing_app_SFDC, "Contact", contact_id,
                                       emailAddress);
    ...
    def getEloquaContact(self, contact_ids={}):
        ...
        for k in contact_ids.keys():
            ...
            if len(response.text) > 0:
                ...
                results["latticeforleads__Score__c"], results["latticeforleads__Last_Score_Date__c"] = self.getScore(
                    result["fieldValues"])
                print "==>    %s    %s    %s    %s" % (k, results["email"], results["latticeforleads__Score__c"],
                                                       results["latticeforleads__Last_Score_Date__c"])
    ...
    def getScore(self, fields=[{}]):
        ...
        score_date = None;
        for field in fields:
            if field["id"] == "100207" and field.has_key("value"):
                score = field["value"]
                print "Lattice Score: ", score
                continue
            elif field["id"] == "100208" and field.has_key("value"):
                score_date = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(float(field["value"])))
                print "Last Score Date: ", score_date
                continue
    ...
    def getLeadFromMarketo(self, lead_ids={}):
        ...
        for k in lead_ids.keys():
            ...
            if response.status_code == 200:
                ...
                if len(results) > 0:
                    result = results[0];
                    print "==>    %s    %s    %d    %s" % (k, result["email"], result["latticeforleads__Score__c"],
                                                           result["latticeforleads__Last_Score_Date__c"]);
                    leads.append(result);
            else:
                print response.text
...
class SFDCRequest():
    def __init__(self, base_url=PLSEnvironments.pls_SFDC_url):
        self.headers = {'Authorization': 'Bearer %s' % self.getAuthToken(), 'Content-type': 'application/json',
    ...
    def getRecords(self, sobjects, record_ids={}):
        ...
        for k in record_ids.keys():
            ...
            if response.status_code == 200:
                ...
                # print result
                print "==>    %s    %s    %s    %s" % (
                    k, result["Email"], result["latticeforleads__Score__c"],
                    result["latticeforleads__Last_Score_Date__c"]);
                result["id"] = result["Id"];
                del result["Id"];
    ...
    def addAccountsToSFDC(self, account_num=3):
        ...
        for address in addresses:
            ...
            if account_id != None:
                ...
                recordNewAdded(sequence, PLSEnvironments.pls_marketing_app_SFDC, "Account", account_id,
                               account_lists[account_id]);
            else:
                failed += 1;
                if failed > 3:
                    break;
    ...
    def updateAccountToSFDC(self, record_id, name=None, BillingCity=None, BillingState=None, BillingCountry=None,
                            Industry=None, AnnualRevenue=None, NumberOfEmployee=None):
        account = {};
        if name != None:
            account["name"] = name;
        if BillingCity != None:
    ...
    def addOpportunityToSFDC(self, account_id=None, opportunity_num=3):
        ...
        for i in range(opportunity_num):
            ...
            if opportunity_id != None:
                ...
                recordNewAdded(sequence, PLSEnvironments.pls_marketing_app_SFDC, "Opportunity", opportunity_id,
                               opportunity_lists[opportunity_id]);
            else:
                failed += 1;
                if failed > 3:
                    break;
    ...
    def addOpportunityConTactRoleToSFDC(self, opportunity_id, contact_id, role_num=3):
        ...
        for i in range(role_num):
            ...
            if role_id != None:
                ...
                recordNewAdded(sequence, PLSEnvironments.pls_marketing_app_SFDC, "OpportunityContactRole", role_id,
                               role_lists[role_id]);
            else:
                failed += 1;
                if failed > 3:
    ...
    def getAuthToken(self, OAuth2_url=PLSEnvironments.pls_SFDC_OAuth2,
        ...
        request_url = "%s?grant_type=password&client_id=%s&client_secret=%s&username=%s&password=%s" % (
            OAuth2_url, client_id, client_secret, user, pwd);
