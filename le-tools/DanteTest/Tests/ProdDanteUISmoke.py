...
class DanteDemoURLTest(unittest.TestCase):
    ...
    def testcase_1_VerifyTalkingPoints(self):
        ...
        infos_TalkingPoints_Expected = danteData_TalkingPoints.GetPRODExpectedTalkingPoints('.\Config\ExpectedResult.xml')
    ...
    def testcase_2_VerifyBuyingSignal(self):
        ...
        infos_BuingSignal_Expected = danteData_BuyingSignal.GetPRODExpectedBuyingSingal('.\Config\ExpectedResult.xml')
...
class DanteInSFDCTest(unittest.TestCase):
    ...
    def testcase_1_VerifyTalkingPoints(self):
        ...
        danteData_TalkingPoints=DanteDataHelper()
        TP_infos_expected=danteData_TalkingPoints.GetPRODExpectedTalkingPoints('.\Config\ExpectedResult.xml')
        dantePage_TalkingPoints.Close_browser()
        ...
        for tp_index in range (0,tps_count):
            ...
            assert ((title_TalkingPoint !='') or (title_TalkingPoint!=None)),'Talking Points Title not exist or is empty in page'
            assert (content_TalkingPoints!=None),'Talking Points Title not exist in page'
    ...
    def testcase_2_VerifyBuyingSignal(self):
        ...
        infos_BuingSignal_Expected=danteData_BuyingSignal.GetPRODExpectedBuyingSingal('.\Config\ExpectedResult.xml')
        dantePage__BuyingSignal.Close_browser()
        assert len(infos_BuingSignal_From_Page)==2, 'Internal and external attributes should be displayed in page,but now only '+str(len(infos_BuingSignal_From_Page)) +' area be displayed.'
