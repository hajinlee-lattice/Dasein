...
from Libs.XMLLibTool import XmlToolLibrary
...
class DanteDataHelper(object):
    ...
    def ParseLeadScore(self,Lead_ID):
        Logger.setInfoLog( '===start to parse Lead score From DB===')
        Dic_Lead={}
        ...
        try:
            ...
            Logger.setExceptionLog("connect or query DB Fail:"+str(e.message))
        finally:
            cursor.close()
            conn.close()
        Logger.setInfoLog( '===end to parse Lead Score and return Lead dict===')
        return Dic_Lead

    def ParseAccountsPlay(self,AccountID):
        Logger.setInfoLog( '===start to parse accounts plays From DB===')
        Dic_Account={}
        ...
        try:
            ...
            cursor.close()
            conn.close()
        Logger.setInfoLog( '===end to parse account plays and return plays dict list===')
        return Dic_Account
            ...
            if dict_Page["DisplayName"]==dict_DB["DisplayName"]:
                if len(dict_Page["Plays"])==len(dict_DB["Plays"]):
                    for play_item in dict_Page["Plays"]:
                        assert self.CheckPlayTitleAndScore(dict_DB["Plays"],play_item["P_Title"],play_item["P_Score"]), Logger.setErrorLog('Play score for the play "'+play_item["P_Title"]+'" in page is not right, actually is: "'+str(play_item["P_Score"]))
                        ...
                        if (tp_title_list_page==tp_title_list_DB):
                            ...
                            Logger.setErrorLog( 'The talking points for play: "'+play_item["P_Title"]+'" in page is not right.\n'+tp_title_list_page+'\n--------'+tp_title_list_DB+'\n--------')
                            result=False
                            break
                else:
                    Logger.setErrorLog( 'The number of plays displayed in page is not right, actually is: '+str(len(dict_Page["Plays"]))+' expected should be: '+str(len(dict_DB["Plays"])))
                    result=False
            else:
                Logger.setErrorLog( 'Display Name in page is not right, actually is: '+dict_Page["DisplayName"]+' expected should be: '+dict_DB["DisplayName"])
                result=False
        return result

    def GetPRODExpectedTalkingPoints(self,filepath):
        Logger.setInfoLog( 'get expected talking points value from xml file')
    ...
    def GetPRODExpectedBuyingSingal(self,filepath):
        Logger.setInfoLog( 'get expected buying signals value from xml file')
        xmldoc=XmlToolLibrary()
        xmldoc.loadXML(filepath)
   ...
   #ddh=DanteDataHelper()
   #dic=ddh.ParseLeadScore('0018000000NW1EEAA1')
   #print dic
