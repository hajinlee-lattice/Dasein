...
class DantePageHelper(object):
    ...
    def LogInSaleForcePage(self):
        ...
        Logger.setInfoLog('===start to log in SalesForce===')
    ...
    def OpenDetailPage(self,D_Type):
        ...
        return Full_ID
        Logger.setInfoLog ('===Have opened '+str(D_Type)+' Detail page===')

    def GoToPlayDetailPage(self,Play_index,ISDetailPage=False):
        Logger.setInfoLog ('======Go to one play detail page by click play======')
        if ISDetailPage:
            Logger.setInfoLog( 'select Dante detail iframe')
            time.sleep(2)
        ...
        if (int(play_count)<int(Play_index)):
            Logger.setInfoLog ('the specific play index: '+ str(Play_index) + ' out of scope, please make sure it is right')
            return False
        else:
            play_dict={}
            ...
            time.sleep(2)
            Logger.setInfoLog ('start to wait TP and BS links exist')
            ...
            Logger.setInfoLog ('one play detail page be opened')
            return play_dict
    ...
    def SetDanteServiceURL(self,D_Type='Lead',LatticeURL=DanteEnvironments.Sales_Force_DT_service,showlift=True,showScore=True,showRating=True,defaultTab='TalkingPoints',hasSalesPrism=False):
        ...
        Logger.setInfoLog('===Start to open configuration page')
        self.OpenConfigurePage(D_Type)
        Logger.setInfoLog ('===Have opened configuration page===')
        if (D_Type=='Lead'):
    ...
    def GetAccountPlay(self,ISDetailPage=False):
        Logger.setInfoLog ('===start to get info for play:===')
        result_dict={}
        if ISDetailPage:
            Logger.setInfoLog( 'select iframe in detail page')
            self.sele_instance.select_frame(DantePageHelper.DantePages["AccountDetailPage"]["Iframe_1"])
    ...
    def GetTalkingPointsForPlay(self,ISDetailPage=False):
        ...
        Logger.setInfoLog ('Talk points link exist in page')
        self.sele_instance.click_link(DantePageHelper.DantePages["VisualAccountPage"]["TPLink"])
        Logger.setInfoLog ('start to select iframe for theIFrame')
        time.sleep(5)
        self.sele_instance.select_frame(DantePageHelper.DantePages["VisualAccountPage"]["Iframe_Play"])
        Logger.setInfoLog ('have selected iframe in play page')
        TP_Result_List=self.GetTalkingPoints()
        #print str(TP_Result_List)
        self.sele_instance.unselect_frame()
        Logger.setInfoLog ('unselect iframe')
        #time.sleep(5)
        Logger.setInfoLog ('---start return play list---')
    ...
    def GetTalkingPoints(self):
        Logger.setInfoLog('Start to get talking points for one play')
        ...
        #print str(TP_count)
        Logger.setInfoLog('The number of talking points are:'+str(TP_count))
        TP_Result_List=[]
        for talkpoints_index in range(1,int(TP_count)+1):
            ...
            #print TP_Title
            Logger.setInfoLog('Talking Points Title is:'+TP_Title)
        ...
        Logger.setInfoLog('Have get all  talking points info')
    ...
    def GetBuyingSignalForPlay(self,ISDetailPage=False):
        ...
        self.sele_instance.unselect_frame()
        if ISDetailPage:
            Logger.setInfoLog( 'select detail iframe')
            self.sele_instance.select_frame(DantePageHelper.DantePages["AccountDetailPage"]["Iframe_1"])
        Logger.setInfoLog( 'Buying Signals link exist in page')
    ...
    def GetActionsForPlay(self,ISPlayDetailPage=False,ISAccountDetailPage=False):
        Logger.setInfoLog( '---Get all actions for account play---')
        result_list=[]
        ...
        if action_count>7:
            Logger.setInfoLog( 'set default count to 7')
            action_count=7
        for action_index in range(1,int(action_count)+1):
            ...
            time.sleep(1)
            if (not ISPlayDetailPage) and (int(action_index)==1):
                self.sele_instance.click_element(DantePageHelper.DantePages["VisualAccountPage"]["ActionButton"])
        if ISAccountDetailPage:
            Logger.setInfoLog( 'unselect iframe in detail page')
            self.sele_instance.unselect_frame()
