...
abstract class QueryTranslator {
    ...
    Restriction translateEntityQueryRestriction(FrontEndQuery frontEndQuery, AttributeRepository attrRepo, //
            TimeFilterTranslator timeTranslator, String sqlUser) {
        Restriction restriction;
        BusinessEntity mainEntity = frontEndQuery.getMainEntity();
        restriction = translateFrontEndRestriction(getEntityFrontEndRestriction(mainEntity, frontEndQuery), attrRepo,
                timeTranslator, sqlUser, true);
        restriction = translateSalesforceIdRestriction(frontEndQuery, mainEntity, restriction);
        restriction = translateInnerRestriction(frontEndQuery, attrRepo, mainEntity, restriction, timeTranslator,
                sqlUser);
        return restriction;
    }
    ...
    Restriction translateFrontEndRestriction(FrontEndRestriction frontEndRestriction, AttributeRepository attrRepo,
        ...
        Restriction restriction = //
                translateFrontEndRestriction(frontEndRestriction, attrRepo, translatePriorOnly, useDepivotedPhTable);
        if (restriction == null) {
            return null;
        }
    ...
    Restriction translateFrontEndRestriction(FrontEndRestriction frontEndRestriction, AttributeRepository attrRepo,
        ...
        Restriction restriction = translateBucketRestriction(frontEndRestriction.getRestriction().getDeepCopy(), //
                attrRepo, translatePriorOnly, useDepivotedPhTable);
    ...
    private Restriction translateInnerRestriction(FrontEndQuery frontEndQuery, AttributeRepository attrRepo,
            BusinessEntity outerEntity, Restriction outerRestriction, TimeFilterTranslator timeTranslator,
            String sqlUser) {
    ...
    Restriction translateInnerRestriction(FrontEndQuery frontEndQuery, AttributeRepository attrRepo,
            BusinessEntity outerEntity, Restriction outerRestriction, boolean useDepivotedPhTable) {
        BusinessEntity innerEntity = null;
        switch (outerEntity) {
        case Contact:
            innerEntity = BusinessEntity.Account;
        ...
        FrontEndRestriction innerFrontEndRestriction = getEntityFrontEndRestriction(innerEntity, frontEndQuery);
        Restriction innerRestriction = translateFrontEndRestriction(innerFrontEndRestriction, attrRepo, true,
                useDepivotedPhTable);
        return addSubselectRestriction(outerEntity, outerRestriction, innerEntity, innerRestriction);
    }
    ...
    private Restriction translateBucketRestriction(Restriction restriction, AttributeRepository attrRepo,
            boolean translatePriorOnly, boolean useDepivotedPhTable) {
        ...
        if (restriction instanceof LogicalRestriction) {
            ...
            search.run(restriction, (object, ctx) -> {
                if (object instanceof BucketRestriction) {
                    ...
                    if (Boolean.TRUE.equals(bucket.getIgnored())) {
                        ...
                        Restriction converted = translateSingleBucket(bucket, attrRepo, translatePriorOnly,
                                useDepivotedPhTable);
            ...
            BucketRestriction bucket = (BucketRestriction) restriction;
            if (!Boolean.TRUE.equals(bucket.getIgnored())) {
                translated = translateSingleBucket(bucket, attrRepo, translatePriorOnly, useDepivotedPhTable);
            } else {
                log.warn("Ignored buckets should be filtered out by optimizer: " + JsonUtils.serialize(bucket));
            }
