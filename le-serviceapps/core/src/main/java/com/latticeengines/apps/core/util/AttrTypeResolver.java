...
public class AttrTypeResolver {
    ...
    static {
        BusinessEntity.SEGMENT_ENTITIES.forEach(e -> internalMap.put(e,
                SchemaRepository.getSystemAttributes(e).stream().map(InterfaceName::name).collect(Collectors.toSet())));
        BusinessEntity.SEGMENT_ENTITIES.forEach(e -> standardMap.put(e, SchemaRepository.getStandardAttributes(e)
