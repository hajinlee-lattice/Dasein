...
def read_contact_base(file):
    ...
    with open(file, 'r') as csvfile:
        ...
        for row in reader:
            ...
            contacts.append(row)
    schema = standardize_schema(schema)
...
def split(list, begin_idx, end_idx):
    return list[begin_idx:end_idx]


def update_account(contacts, accounts):
    for k in range(len(contacts)):
        contact = contacts[k]
        i = k % len(list(ContactToAccount))
        acct_website = accounts[contact[CUSTOMER_ACCOUNT_ID]][WEBSITE]
        acct_company_name = accounts[contact[CUSTOMER_ACCOUNT_ID]][ACC_COMPANY_NAME]
        acct_company_country = accounts[contact[CUSTOMER_ACCOUNT_ID]][COUNTRY]
        acct_company_state = accounts[contact[CUSTOMER_ACCOUNT_ID]][STATE]
        acct_company_city = accounts[contact[CUSTOMER_ACCOUNT_ID]][CITY]
        acct_postal_code = accounts[contact[CUSTOMER_ACCOUNT_ID]][ACC_POSTAL_CODE]
        # MatchKey = AID
...
def output(contacts, file, schema):
...
if __name__ == '__main__':
    # dict: AID -> Account
    accounts = read_account_base(ACCOUNT_FILE)
    # list: Contact
    ...
    update_account(contacts1, accounts)
    update_account(contacts3, accounts)
