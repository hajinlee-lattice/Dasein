...
public class ProcessAnalyzeWorkflowSubmitter extends WorkflowSubmitter {
    ...
    public ApplicationId submit(String customerSpace) {
        ...
        if (!datafeedStatus.isAllowConsolidation()) {
            throw new RuntimeException("We can't launch any process analyze workflow now as it is not ready.");
        } else if (execution != null && DataFeedExecution.Status.Started.equals(execution.getStatus())) {
            if (execution.getWorkflowId() == null) {
                throw new RuntimeException(
                        "We can't launch any process analyze workflow now as there is one still running.");
            }
