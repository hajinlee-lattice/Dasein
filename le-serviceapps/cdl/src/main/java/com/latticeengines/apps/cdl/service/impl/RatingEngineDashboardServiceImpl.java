...
public class RatingEngineDashboardServiceImpl extends RatingEngineTemplate
        implements RatingEngineDashboardService {
    ...
    public RatingEngineDashboard getRatingsDashboard(String customerSpace, String ratingEngineId) {
        ...
        RatingEngine ratingEngine = ratingEngineService.getRatingEngineById(ratingEngineId, true,
                true);
        RatingEngineSummary ratingEngineSummary = constructRatingEngineSummary(ratingEngine,
                customerSpace);
        log.info(String.format("Step 1 - Loading rating engine summary completed for : %s",
                ratingEngineId));

        // get coverage info
        CoverageInfo coverageInfo = RatingEngineUtils.getCoverageInfo(ratingEngine);
        log.info(String.format("Step 2 - Loading ratings coverage completed for : %s",
                ratingEngineId));
        ...
        log.info(
                String.format("Step 3 - Loading related plays completed for : %s", ratingEngineId));
        ...
        log.info(String.format("Step 3.1 - Loading related dependencies completed for : %s",
                ratingEngineId));
        ...
        List<RatingModelDTO> iterations = ratingModels.stream().map(rm -> new RatingModelDTO(rm))
                .collect(Collectors.toList());

        log.info(String.format("Step 4 - Loading related model iterations for : %s",
                ratingEngineId));
