...
public class AppmasterRmTemplate extends YarnRpcAccessor<ApplicationMasterProtocol> implements AppmasterRmOperations {
    ...
    public RegisterApplicationMasterResponse registerApplicationMaster(final String host, final Integer rpcPort,
        ...
        return execute(new YarnRpcCallback<RegisterApplicationMasterResponse, ApplicationMasterProtocol>() {
            ...
            public RegisterApplicationMasterResponse doInYarn(ApplicationMasterProtocol proxy)
                ...
                return amRMClient.registerApplicationMaster(appMasterRequest.getHost(), appMasterRequest.getRpcPort(),
                        appMasterRequest.getTrackingUrl());
            }
        });
    }
    ...
    public FinishApplicationMasterResponse finish(final FinishApplicationMasterRequest request) {
        return execute(new YarnRpcCallback<FinishApplicationMasterResponse, ApplicationMasterProtocol>() {
            @Override
            public FinishApplicationMasterResponse doInYarn(ApplicationMasterProtocol proxy)
                    throws YarnException, IOException {
                amRMClient.unregisterApplicationMaster(request.getFinalApplicationStatus(), request.getDiagnostics(),
    ...
    private static void setupTokens(InetSocketAddress resourceManagerAddress) throws IOException {
        ...
        for (Token<? extends TokenIdentifier> token : UserGroupInformation.getCurrentUser().getTokens()) {
            if (token.getKind().equals(AMRMTokenIdentifier.KIND_NAME)) {
                ...
                // the appropriate service-name. We'll need more infrastructure
                // when
