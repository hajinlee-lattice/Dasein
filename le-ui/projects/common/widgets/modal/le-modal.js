		...
		this.unsubscribe = props.store.subscribe(() => {
			...
			this.setState({
				...
				titleIcon: data.config.titleIcon
					? data.config.titleIcon
					: undefined,
				hideFooter: data.config.hideFooter
					? data.config.hideFooter
					: false,
				type: data.config.type ? data.config.type : "",
				oneButton: data.config.oneButton
					? data.config.oneButton
					: false,
				size: data.config.size ? data.config.size : MEDIUM_SIZE,
				cancelLabel: data.config.cancelLabel
					? data.config.cancelLabel
					: "Cancel",
				confirmLabel: data.config.confirmLabel
					? data.config.confirmLabel
					: "Done"
			});
		});
		this.clickHandler = this.clickHandler.bind(this);
		...
		if (!this.state.oneButton) {
			return (
				<LeButton
					...
					callback={() => {
						return this.clickHandler("close");
					}}
				/>
			);
		} else {
	...
	getModalUI() {
		return (
			<div className="le_modal opened">
				<div
					className={`${"le-modal-content"} ${this.state.size} ${
				...
				>
					...
					>
						...
						<div className="le-title-container">
							{this.getTitleIcon()}
							<p className="le-title">{this.getTitle()}</p>
						</div>
					</div>
