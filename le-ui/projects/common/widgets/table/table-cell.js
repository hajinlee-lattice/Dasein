...
export default class LeTableCell extends Component {
	...
	getCellContent() {
		let displayName = getColumnData(
			this.props.rowData,
			this.props.colName.split(".")
		); //this.props.rowData[this.props.colName];

		if (
			displayName &&
			!this.state.editing &&
			this.props.columnsMapping[this.props.colName].onlyTemplate != true
		) {
			// console.log('================');
