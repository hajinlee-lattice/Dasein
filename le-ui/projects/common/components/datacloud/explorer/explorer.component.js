...
angular
    ...
    .controller('DataCloudController', function (
        ...
        vm.init = function () {
            ...
            if (
                $state.current.name ===
                'home.ratingsengine.dashboard.segment.attributes.add' ||
                $state.current.name ===
            ...
            if (
                ...
                QueryStore.getAddBucketTreeRoot()
            ) {
            ...
            if (vm.section == 'insights' && !vm.show_lattice_insights) {
                vm.statusMessage(vm.label.insufficientUserRights, {
                    wait: 0,
                    special: 'nohtml'
                });
                return false;
            ...
            if (
                vm.lookupMode &&
                typeof vm.lookupFiltered === 'object' &&
                (Object.keys(vm.lookupFiltered).length < 1 ||
                    LookupStore.hideLookupResponse(LookupResponse))
            ) {
            ...
            $scope.$on('$destroy', function () {
                delete (vm.addBucketTreeRoot = QueryStore.setAddBucketTreeRoot(
                    null
                ));
            });
        ...
        vm.processEnrichments = function (enrichments) {
            if (vm.lookupFiltered !== null) {
                for (
                    var i = 0, _enrichments = [];
                    i < enrichments.length;
                    i++
                ) {
                    if (
                        vm.lookupFiltered &&
                        vm.lookupFiltered[enrichments[i].ColumnId]
                    ) {
            ...
            for (var i = 0, enrichment; i < _enrichments.length; i++) {
                ...
                enrichment.Entity = enrichment.Entity || 'Account';

                vm.enrichmentsMap[
                    enrichment.Entity + '.' + enrichment.ColumnId
                ] = vm.enrichments.length;
            ...
            var selectedTotal = vm.filter(vm.enrichments, 'IsSelected', true),
                EligibleEnrichments = vm.filter(
                    vm.enrichments,
                    'IsInternal',
                    false
                ),
                DisabledForSalesTeamTotal = vm.filter(
                    EligibleEnrichments,
                    'AttributeFlagsMap.CompanyProfile.hidden',
                    true
                ),
                EnabledForSalesTeamTotal =
                    EligibleEnrichments.length -
                    DisabledForSalesTeamTotal.length;

            if (vm.lookupMode) {
                LookupStore.add('count', vm.enrichments.length);
            } else {
                DataCloudStore.setMetadata(
                    'generalSelectedTotal',
                    selectedTotal.length
                );
                DataCloudStore.setMetadata(
                    'premiumSelectedTotal',
                    vm.filter(selectedTotal, 'IsPremium', true).length
                );
                DataCloudStore.setMetadata(
                    'enabledForSalesTeamTotal',
                    EnabledForSalesTeamTotal
            ...
            vm.generalSelectedTotal = DataCloudStore.getMetadata(
                'generalSelectedTotal'
            );
            vm.premiumSelectedTotal = DataCloudStore.getMetadata(
                'premiumSelectedTotal'
            );
        };
        ...
        vm.generateTree = function (isComplete) {
            ...
            vm.enrichments.forEach(function (item, index) {
                ...
                item.HighlightHidden =
                    item.AttributeFlagsMap &&
                        ...
                        item.AttributeFlagsMap.CompanyProfile.hidden === true
                        ? true
                        : false;
                item.HighlightHighlighted =
                    item.AttributeFlagsMap &&
                        item.AttributeFlagsMap.CompanyProfile &&
                        item.AttributeFlagsMap.CompanyProfile.highlighted
                        ? item.AttributeFlagsMap.CompanyProfile.highlighted
                        : false;

                if (vm.section == 'wizard.ratingsengine_segment') {
                    item.IsRatingsEngineAttribute = ratingsEngineAttributeState(
                        item
                    );
                }
            ...
            if (isComplete) {
                ...
                if (
                    vm.section === 'wizard.ratingsengine_segment' ||
                    vm.section === 'dashboard.rules'
                ) {
                    var SelectedForRatingsEngine = vm.filter(
                        vm.enrichments,
                        'IsRatingsEngineAttribute',
                        true
                    );
                    DataCloudStore.setMetadata(
                        'selectedForRatingsEngine',
                        SelectedForRatingsEngine.length
                    );
                    RatingsEngineStore.setValidation(
                        'attributes',
                        SelectedForRatingsEngine.length > 0
                    );
                }
        ...
        vm.filterEmptySubcategories = function () {
            if (vm._subcategories[vm.category]) {
                for (
                    var i = 0, newCategories = [];
                    i < vm._subcategories[vm.category].length;
                    i++
                ) {
        ...
        var breakOnFirstEncounter = function (
            items,
            property,
            value,
            returnObj
            ...
            for (var i = 0, item; i < items.length; i++) {
                ...
                if (typeof value === 'object') {
                    if (
                        typeof items[i][property] === 'object' &&
            ...
            vm.categories.forEach(function (category) {
                if (vm.enrichmentsObj && vm.enrichmentsObj[category]) {
                    ...
                    var items = vm.enrichmentsObj[category],
                        disabled = breakOnFirstEncounter(
                            items,
                            'HighlightHidden',
                            true
                        ),
                        enabled = breakOnFirstEncounter(
                            items,
                            'HighlightHidden',
                            false
                        ),
                        dirty = breakOnFirstEncounter(
                            items,
                            'AttributeFlagsMap',
                            {}
                        );
                    ...
                    vm.highlightMetadata.categories[category].enabled = enabled
                        ? 1
                        : 0;
                    vm.highlightMetadata.categories[
                        category
                    ].disabled = disabled ? 1 : 0;
                    vm.highlightMetadata.categories[category].dirty = dirty
                        ? 1
                        : 0;

                    if (
                        vm.subcategories[category] &&
                        vm.subcategories[category].length > 1
                    ) {
                        vm.highlightMetadata.categories[
                            category
                        ].subcategories = {};

                        vm.subcategories[category].forEach(function (
                            subcategory
                        ) {
                            vm.highlightMetadata.categories[
                                category
                            ...
                            vm.highlightMetadata.categories[
                                category
                            ].subcategories[subcategory].enabled = enabled
                                    ? 1
                                    : 0;
                            vm.highlightMetadata.categories[
                                category
                            ].subcategories[subcategory].disabled = disabled
                                    ? 1
                                    : 0;
                            vm.highlightMetadata.categories[
                                category
                            ].subcategories[subcategory].dirty = dirty ? 1 : 0;
                        });
                    }
                }
            });
        ...
        var highlightOptionsInitState = function (enrichment) {
            ...
            if (
                enrichment.AttributeFlagsMap.CompanyProfile.highlighted === true
            ) {
        ...
        var ratingsEngineAttributeState = function (item) {
            ...
            if (
                ratingsEngineAttributes.indexOf(
                    item.Entity + '.' + item.ColumnId
                ) >= 0
            ) {
                return true;
            }
        ...
        vm.setFlags = function (type, enrichment) {
            ...
            setFlags(opts, flags).then(function () {
                ...
                enrichment.HighlightState = {
                    type: type,
                    label: label,
                    enabled: !flags.hidden,
                ...
                var EligibleEnrichments = vm.filter(
                    vm.enrichments,
                    'IsInternal',
                    false
                ),
        ...
        var setFlagsByCategory = function (opts, flags) {
            ...
            if (opts.subcategoryName) {
                DataCloudService.setFlagsBySubcategory(opts, flags).then(
                    function (result) {
                        deferred.resolve(result);
                    }
                );
            } else if (opts.categoryName) {
                DataCloudService.setFlagsByCategory(opts, flags).then(function (
                    result
                ) {
                    deferred.resolve(result);
                });
        ...
        vm.setFlagsByCategory = function (type, category, subcategory) {
            var opts = {
                ...
                settings_changed =
                    vm.highlightTypesCategoryLabel(category, subcategory)
                        .type !== type;
            ...
            setFlagsByCategory(opts, flags).then(function () {
                ...
                for (var i in enrichments) {
                    var enrichment = enrichments[i],
                        flags = {},
                        label = enrichment.HighlightHighlighted
                            ? enrichment.HighlightState.label
                            : vm.highlightTypes[type],
                    ...
                    enrichment.AttributeFlagsMap.CompanyProfile.hidden =
                        flags.hidden;
                    enrichment.HighlightHidden = flags.hidden;
        ...
        var getTopAttributes = function (opts) {
            ...
            DataCloudStore.getAllTopAttributes().then(function (result) {
                ...
                Object.keys(EnrichmentTopAttributes).forEach(function (
                    catKey,
                    catItem
                ) {
                    var category =
                        EnrichmentTopAttributes[catKey]['Subcategories'];
        ...
        vm.processCategories = function () {
            ...
            if (
                (vm.show_segmentation && vm.section == 'segment.analysis') ||
                vm.section == 'wizard.ratingsengine_segment' ||
                vm.section == 'dashboard.rules'
            ) {
        ...
        vm.getTileTableItems = function (
            ...
            if (vm.topAttributes[category]) {
                ...
                if (!subcategory) {
                    //PLS-4922
                    Object.keys(
                        vm.topAttributes[category].Subcategories
                    ).forEach(function (key, index) {
                        items = items.concat(
                            vm.topAttributes[category].Subcategories[key]
                        );
                    });
                } else {
                    items =
                        vm.topAttributes[category].Subcategories[subcategory];
                }
            ...
            if (vm.lookupMode || !items || items.length == 0) {
                ...
                if (subcategory || vm.isYesNoCategory(category, true)) {
                    items = items.filter(function (item) {
                        var isSubcategory = subcategory
                            ? item.Subcategory == subcategory
                            : true,
                            attrValue = vm.lookupFiltered
                                ? vm.lookupFiltered[item.ColumnId]
                                : item.AttributeValue;
        ...
        vm.inCategory = function (enrichment) {
            ...
            var selected =
                typeof vm.selected_categories[enrichment.Category] === 'object';
        ...
        vm.selectEnrichment = function (enrichment) {
            ...
            if (enrichment.IsPremium) {
                vm.premiumSelectedTotal = vm.filter(
                    selectedTotal,
                    'IsPremium',
                    true
                ).length;
            ...
            if (enrichment.IsSelected) {
                ...
                if (!enrichment.WasDirty) {
                    ...
                    var notselected = vm.filter(
                        vm.enrichments,
                        'IsSelected',
                        false
                    );
                    vm.disabled_count = vm.filter(
                        notselected,
                        'IsDirty',
                        true
                    ).length;
                    vm.label.disabled_alert =
                        '<p><strong>You have disabled ' +
                        vm.disabled_count +
                        ' attribute' +
                        (vm.disabled_count > 1 ? 's' : '') +
                        '</strong>. If you are using any of these attributes for real-time scoring, these attributes will no longer be updated in your system.</p>';
                    vm.label.disabled_alert +=
                        "<p>No changes will be saved until you press the 'Save' button.</p>";
        ...
        vm.hasSubcategories = function (category) {
            return (
                vm._subcategories[category].length != 1 ||
                vm._subcategories[category][0] != 'Other'
            );
        };

        vm.checkSelectedRatingEngineAttrs = function () {
            return typeof vm.metadata.toggle.show
                .selected_ratingsengine_attributes === 'undefined'
                ? false
        ...
        var getExplorerSegments = function (enrichments) {
            if (vm.metadataSegment != undefined) {
                var accountRestrictions =
                    vm.metadataSegments.accountRestrictions,
                    contactRestrictions =
                        vm.metadataSegments.contactRestrictions;
            } else {
            ...
            accountRestrictions = accountRestrictions
                ? angular.copy(
                    accountRestrictions.restriction.logicalRestriction
                        .restrictions
                )
                : [];
            ...
            contactRestrictions = contactRestrictions
                ? angular.copy(
                    contactRestrictions.restriction.logicalRestriction
                        .restrictions
                )
                : [];
            ...
            for (var i = 0; i < restrictions.length; i++) {
                ...
                var restriction = restriction.bucketRestriction,
                    ...
                    enrichment = breakOnFirstEncounter(
                        vm.enrichments,
                        'ColumnId',
                        key,
                        true
                ...
                if (index || index === 0) {
                    ...
                    vm.segmentAttributeInput[
                        vm.enrichments[index].ColumnId
                    ] = true;
                    vm.segmentAttributeInputRange[
                        vm.makeSegmentsRangeKey(enrichment, range, label)
                    ] = true;
                }
            }
        };
        ...
        vm.searchFields = function (enrichment) {
            if (vm.query) {
                if (
                    ...
                    textSearch(enrichment.DisplayName, vm.query)
                ) {
                    return true;
                } else if (
                    enrichment.Description &&
                    textSearch(enrichment.Description, vm.query)
                ) {
                    return true;
                } else if (
                    enrichment.Subcategory &&
                    textSearch(enrichment.Subcategory, vm.query)
                ) {
                    return true;
                } else {
                    return false;
        ...
        var getSegmentBucketInputs = function () {
            ...
            if (metadataSegments && metadataSegments.all) {
                metadataSegments.all.forEach(function (item) {
                    var bucketId =
                        item.bucketRestriction.lhs.columnLookup.column_name +
                        item.bucketRestriction.range.max;
        ...
        vm.segmentAttributeInput =
            DataCloudStore.getMetadata('segmentAttributeInput') || {};
        vm.selectSegmentAttribute = function (attribute) {
            ...
            if (!attribute.TopBkt && !vm.inWizard) {
                vm.addFreeTextAttribute(
                    attribute,
                    vm.cube.data[attribute.Entity].Stats[attribute.Attribute]
                );
                return;
            }
            ...
            var attribute = angular.copy(attribute),
                ...
                attributeRangeKey = stat.Lbl
                    ? vm.makeSegmentsRangeKey(attribute, stat.Rng, stat.Lbl)
                    : '',
                index =
                    vm.enrichmentsMap[attribute.Entity + '.' + attributeKey],
                enrichment = vm.enrichments[index],
                entity = attribute.Entity,
                topBkt = attribute.TopBkt,
                segmentName = $stateParams.segment;

            vm.segmentAttributeInput[attributeKey] = !vm.segmentAttributeInput[
                attributeKey
            ];
            DataCloudStore.setMetadata(
            ...
            if (attributeRangeKey) {
                vm.segmentAttributeInputRange[attributeRangeKey] = !vm
                    .segmentAttributeInputRange[attributeRangeKey];
            }

            var addEntity =
                entity == 'Rating' ||
                    entity == 'LatticeAccount' ||
                    entity == 'CuratedAccount'
                    ? 'Account'
                    : entity;
        ...
        vm.selectSegmentAttributeRange = function (enrichment, stat, disable) {
            var disable = disable || false,
                attributeKey =
                    ...
                    enrichment.ColumnId ||
                    enrichment.ColumnId,
                attributeRangeKey = vm.makeSegmentsRangeKey(
                    enrichment,
                    stat.Rng,
                    stat.Lbl
                ),
        ...
        vm.addFreeTextAttribute = function (enrichment, cube) {
            ...
            var bkt = {
                //default bucket for free-text attributes added in My Data or Add step of rules-based rating engine
                Cmp: 'IS_NOT_NULL',
                Cnt: count,
                Id: -1,
                Lbl: '*',
        ...
        vm.selectBucketInput = function (id, bucket) {
            ...
            if (vm.segmentBucketInput[bucketId] === true) {
                QueryStore.addAccountRestriction({
                    columnName: id,
                    range: range
                });
            } else {
                QueryStore.removeAccountRestriction({
                    columnName: id,
                    range: range
                });
            }
        ...
        vm.selectRatingsEngineAttribute = function (enrichment) {
            ...
            if (treeRoot) {
                var enrichments =
                    vm.topAttributes[enrichment.Category].Subcategories[
                    enrichment.Subcategory
                    ],
                ...
                DataCloudStore.selectRatingsEngineAttributes(
                    $stateParams.rating_id,
                    rule.id,
                    [enrichment.Entity + '.' + enrichment.ColumnId]
                ).then(function (response) {
                    ExplorerUtils.removeAddAttrFromRule(
                        !enrichment.IsRatingsEngineAttribute,
                        rule,
                        entity,
                        attr
                    );
        ...
        vm.getAttributeRules = function (attribute, bucket) {
            ...
            var attributes = QueryStore.getDataCloudAttributes(
                true,
                getEmptyBuckets
            ); // second parm is getEmptyBuckets
            ...
            attributes = attributes.filter(function (item) {
                var restriction = item.bucketRestriction,
                    isSameAttribute =
                        ...
                        attribute.Entity +
                        '.' +
                        (attribute.Attribute || attribute.ColumnId),
                    isSameBucket = true,
                    bkt = restriction.bkt,
                    ret = isSameAttribute;

                if (bucket && bkt) {
                    ret = QueryTreeService.getAttributeRules(
                        restriction,
                        bkt,
                        bucket,
                        isSameAttribute
        ...
        vm.formatAttributeValue = function (attribute, rules) {
            var label = attribute.TopBkt
                ? attribute.TopBkt.Lbl
                : attribute.Value;
    ...
    .directive('attributeFeedbackModal', function () {
        return {
            restrict: 'EA',
            templateUrl:
                '/components/datacloud/explorer/attributefeedback/attributefeedbackmodal.component.html',
            scope: {
                lookupResponse: '=?'
            },
            controller: [
                ...
                function (
                    ...
                    $scope.showLookupStore =
                        $scope.modal.context.showLookupStore;
                    ...
                    $scope.report = function () {
                        ...
                        var report = {
                            ...
                            MatchedKeys: {
                                LDC_Name:
                                    $scope.lookupResponse.attributes.LDC_Name,
                                LDC_Domain:
                                    $scope.lookupResponse.attributes.LDC_Domain,
                                LDC_City:
                                    $scope.lookupResponse.attributes.LDC_City,
                                LDC_State:
                                    $scope.lookupResponse.attributes.LDC_State,
                                LDC_ZipCode:
                                    $scope.lookupResponse.attributes
                                        .LDC_ZipCode,
                                LDC_Country:
                                    $scope.lookupResponse.attributes
                                        .LDC_Country,
                                LDC_DUNS:
                                    $scope.lookupResponse.attributes.LDC_DUNS
                            },
                        ...
                        DataCloudStore.sendFeedback(
                            report,
                            $scope.modal.context.type
                        );
                    };
                    ...
                    var setHeight = function () {
                        ...
                        $modal
                            .find('.attribute-feedback-container')
                    ...
                    var _handleDocumentResize = _.debounce(
    ...
    .directive('attributeFeedbackButton', function () {
        return {
            restrict: 'EA',
            templateUrl:
                '/components/datacloud/explorer/attributefeedback/attributefeedbackbutton.component.html',
            scope: {
                buttonType: '=?',
                attribute: '=',
                attributeKey: '=?',
            ...
            controller: [
                ...
                '$stateParams',
                'DataCloudStore',
                function ($scope, $stateParams, DataCloudStore) {
