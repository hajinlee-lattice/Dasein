...
angular
	.module("lp.import.wizard.accountids", [])
	.controller("ImportWizardAccountIDs", function(
		...
		angular.extend(vm, {
			...
			keyMap: {},
			saveMap: {},
			matchIdItems: ImportWizardStore.getMatchIdsItems(
				FieldDocument.fieldMappings
			),
			systems: [],
			match: false
		});
			...
			for (var i in mapping) {
				if (mapping[i] || mapping[i] === "") {
					...
					if (
						...
						map.mappedField == "CustomerAccountId"
					) {
		...
		vm.removeMatchId = index => {
			...
			for (var i = 0; i < mapped.length - 1; i++) {
				if (
					mapped[i].userField == ufName &&
					mapped[i].SystemName == sName
				) {
					mapped.splice(i, 1);
					return;
				}
		...
		vm.isMatchDisabled = fieldMapping => {
			if (fieldMapping && fieldMapping.account == "") {
				vm.match = false;
				vm.changeLatticeField(vm.fieldMapping, vm.form);
				return true;
			} else {
