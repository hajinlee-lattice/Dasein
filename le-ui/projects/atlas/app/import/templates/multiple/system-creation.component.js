...
export default class SystemCreationComponent extends Component {
	...
	componentDidMount() {
		if (this.systemsObj.length > 0) {
			this.setState({ systemSelected: this.systemsObj[0] });
		}
	}
	componentWillUnmount() {}
	validate() {
		if (
			this.state.systemSelected &&
			this.state.newSystemName.trim() != ""
		) {
			...
			systems.push(
				<div
					...
					className={`${"system-card-fix"} ${
						(this.state.systemSelected &&
							this.state.systemSelected.name) == system.name
							? "selected"
							: ""
					}`}
				>
					<LeCard classNames={`${"system-card"}`}>
						<LeCardImg
							src={`${
								(this.state.systemSelected &&
									this.state.systemSelected.name) ==
								system.name
									? system.img_select
									: system.img
							}`}
							classNames="system-image"
						/>
					</LeCard>
				</div>
			);
				...
				>
					...
					>
						...
						<p className="le-description">
							The system will automatically create default field
							mappings between Lattice and Salesforce. The system
							will automatically create 3 separate field mapping
							one each for Salesforce Accounts, Contacts & Leads.
							<br />
							<br />
							These field mappings are here to help you get you
							started quickly and can be edited anytime.
						</p>

						<LeHPanel className="system-info" hstretch={true}>
							<LeSimpleInputText
								config={{ label: "System Name" }}
								callback={val => {
									this.setState(
										{ newSystemName: val },
										this.validate
