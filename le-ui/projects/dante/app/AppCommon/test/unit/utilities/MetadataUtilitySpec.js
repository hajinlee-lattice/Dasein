...
describe('MetadataUtility Tests', function () {
    ...
    describe('GetNotionPropertiesOfKeyValue given valid parameters', function () {
        it('should return all properties with PropertyTypeString given a valid Property Type', function () {
            var notionMetadata = metadataUtil.GetNotionMetadata("DanteAccount", sourceMetadata);
            var segmentProperties = metadataUtil.GetNotionPropertiesOfKeyValue("PropertyTypeString", metadataUtil.PropertyType.SEGMENT, notionMetadata);

            expect(segmentProperties.length).toBe(2);
            segmentProperties.forEach(function (property) {
                expect(property.PropertyTypeString).toBe(metadataUtil.PropertyType.SEGMENT)
            });
        });
    });
});
