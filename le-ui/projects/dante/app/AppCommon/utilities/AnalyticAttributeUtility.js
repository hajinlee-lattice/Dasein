...
.service('AnalyticAttributeUtility', function (ResourceUtility, DateTimeFormatUtility, NumberUtility) {
        ...
        for (var x = 0; x < predictor.Elements.length; x++) {
            ...
            if (bucket.Values != null && bucket.Values.length > 0) {
                ...
                for (var y = 0; y < bucket.Values.length; y++) {
                    // There is a current mismatch occurring where AttributeValues are appearing as Yes/No and
                    // the bucket values are 1.0/0.0 so we need to correctly match them (ENG-8152)
                    if (attributeData.AttributeValue == bucket.Values[y] ||
                        attributeData.AttributeValue === "Yes" && bucket.Values[y] === "1.0" ||
                        attributeData.AttributeValue === "No" && bucket.Values[y] === "0.0" ||
                        attributeData.AttributeValue === "0" && bucket.Values[y] === "0.0" ||
                        attributeData.AttributeValue === "1" && bucket.Values[y] === "1.0" ) {
                        return bucket;
                    }
                }
            } else {
    ...
    this.IsAllowedForInsights = function (attributeMetadata) {
        if (attributeMetadata == null || attributeMetadata.ApprovedUsage == null || attributeMetadata.ApprovedUsage.length === 0) {
            return false;
        } else {
