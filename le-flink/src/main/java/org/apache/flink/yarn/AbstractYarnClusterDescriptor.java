...
public abstract class AbstractYarnClusterDescriptor implements ClusterDescriptor<YarnClusterClient> {
	...
	public AbstractYarnClusterDescriptor(YarnConfiguration yarnConfiguration) {
	...
	public abstract void setExtraJars(Map<String, String> extraJars);

	public abstract Map<String, String> getExtraJars();
...
//		}
	...
	protected YarnClusterClient deployInternal() throws Exception {
		...
		Map<String, String> extraJars = getExtraJars();
		if (extraJars != null) {
			LOG.info("Adding extra jar ...");
			for (Map.Entry<String, String> entry: extraJars.entrySet()) {
				String jar = entry.getValue();
				String jarId = entry.getKey();
				Path jarPath = new Path(jar);
				LocalResource lr = Records.newRecord(LocalResource.class);
				if (FlinkConstants.AM_JAR.equals(jarId)) {
					LOG.info("Found flink.jar at " + jarPath);
					String suffix = ".flink/" + appId + "/" + jarPath.getName();
					remotePathJar = new Path(fs.getHomeDirectory(), suffix);
					LOG.info("Copying from " + jarPath + " to " + remotePathJar);
					HdfsUtils.copyFiles(conf, jarPath.toString(), remotePathJar.toString());
					jarPath = remotePathJar;
				}
				Utils.registerLocalResource(fs, jarPath, lr);
				paths.add(jarPath);
				localResources.put(jarId, lr);
				classPathBuilder.append(jarId).append(File.pathSeparator);
				LOG.info("Added " + jar + " as resource " + jarId);
			}
		}

		// Setup jar for ApplicationMaster
		if (!localResources.containsKey(FlinkConstants.AM_JAR)) {
			LocalResource appMasterJar = Records.newRecord(LocalResource.class);
			remotePathJar =
					Utils.setupLocalResource(fs, appId.toString(), flinkJarPath, appMasterJar, fs.getHomeDirectory());
			localResources.put(FlinkConstants.AM_JAR, appMasterJar);
			paths.add(remotePathJar);
			classPathBuilder.append(FlinkConstants.AM_JAR).append(File.pathSeparator);
		}

		if (!localResources.containsKey("flink-conf.yaml")) {
		...
		if (remotePathJar == null) {
			throw new RuntimeException("Did not find flink.jar after loading extra jars and local jars.");
		}
