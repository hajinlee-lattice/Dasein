...
public class ScoringApiControllerDeploymentTestNGBase extends ScoringApiFunctionalTestNGBase {
    ...
    protected static final String LOCAL_MODEL_PATH = "com/latticeengines/scoringapi/model/"
            + TEST_MODEL_FOLDERNAME + "/";
    ...
    private static final String MODELSUMMARYJSON_LOCALPATH = LOCAL_MODEL_PATH
            + ModelRetrieverImpl.MODEL_JSON;
    private static final Log log = LogFactory
            .getLog(ScoringApiControllerDeploymentTestNGBase.class);
    ...
    public void beforeClass() throws IOException {
        ...
        if (shouldInit()) {
            ...
            if (shouldUseAppId()) {
                ...
                oAuth2RestTemplate = latticeOAuth2RestTemplateFactory.getOAuth2RestTemplate(
                        oAuthUser, CLIENT_ID_LP, getAppIdForOauth2(), authHostPort);
            } else {
                oAuth2RestTemplate = OAuth2Utils.getOauthTemplate(authHostPort,
                        oAuthUser.getUserId(), oAuthUser.getPassword(), CLIENT_ID_LP);
            }
    ...
    private void setupHdfsArtifacts(Tenant tenant) throws IOException {
        ...
        String artifactTableDir = String.format(
                ModelRetrieverImpl.HDFS_SCORE_ARTIFACT_EVENTTABLE_DIR, tenantId, EVENT_TABLE);
        String artifactBaseDir = String.format(ModelRetrieverImpl.HDFS_SCORE_ARTIFACT_BASE_DIR,
                tenantId, EVENT_TABLE, MODEL_VERSION, PARSED_APPLICATION_ID);
        ...
        URL eventTableDataCompositionUrl = ClassLoader.getSystemResource(
                LOCAL_MODEL_PATH + "eventtable-" + ModelJsonTypeHandler.DATA_COMPOSITION_FILENAME);
        URL modelJsonUrl = ClassLoader.getSystemResource(MODELSUMMARYJSON_LOCALPATH);
        URL rfpmmlUrl = ClassLoader
                .getSystemResource(LOCAL_MODEL_PATH + ModelJsonTypeHandler.PMML_FILENAME);
        ...
        URL scoreDerivationUrl = ClassLoader.getSystemResource(
                LOCAL_MODEL_PATH + ModelJsonTypeHandler.SCORE_DERIVATION_FILENAME);
        ...
        String eventTableDataCompositionContents = Files.toString(
                new File(eventTableDataCompositionUrl.getFile()), Charset.defaultCharset());
        eventTableDataComposition = JsonUtils.deserialize(eventTableDataCompositionContents,
                DataComposition.class);

        String dataScienceDataCompositionContents = Files.toString(
                new File(dataScienceDataCompositionUrl.getFile()), Charset.defaultCharset());
        dataScienceDataComposition = JsonUtils.deserialize(dataScienceDataCompositionContents,
                DataComposition.class);
    }
    ...
    protected ScoreRequest getScoreRequest(boolean isPmmlModel) throws IOException {
        ...
        if (isPmmlModel) {
            ...
            URL scoreRequestUrl = ClassLoader.getSystemResource(
                    isPmmlModel ? null : LOCAL_MODEL_PATH + "score_request.json");
            scoreRecordContents = Files.toString(new File(scoreRequestUrl.getFile()),
    ...
    protected void checkModelDetails(List<ModelDetail> models, String modelNamePrefix,
            String fieldDisplayNamePrefix) throws ParseException {
        ...
        for (ModelDetail model : models) {
            ...
            checkFields(model.getModel().getName(), model.getFields(), modelNamePrefix,
                    fieldDisplayNamePrefix);
        }
    }

    protected void checkFields(String modelName, Fields fields, String modelNamePrefix,
            String fieldDisplayNamePrefix) {
