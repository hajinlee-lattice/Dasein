...
public class ScoreRequestProcessorImpl extends BaseRequestProcessorImpl implements ScoreRequestProcessor {
    ...
    public ScoreResponse process(CustomerSpace space, ScoreRequest request, boolean isDebug) {
        ...
        requestInfo.put("ModelType", (scoringArtifacts.getModelType() == null ? "" : scoringArtifacts.getModelType()
                .name()));
    ...
    public List<RecordScoreResponse> process(CustomerSpace space, BulkRecordScoreRequest request, boolean isDebug) {
        ...
        try {
            ...
            if (!partiallyOrderedPmmlParsedRecordList.isEmpty()) {
                Map<RecordModelTuple, Map<String, Object>> unorderedFormattedPmmlRecordMap = format(partiallyOrderedPmmlParsedRecordList);
    ...
    private void extractParsedTuple(Map<String, Entry<LedpException, ScoringArtifacts>> scoringArtifactsMap, //
        ...
        } else if (ModelJsonTypeHandler.PMML_MODEL.equals(scoringArtifactsMap.get(tuple.getModelId()).getValue()
                .getModelJsonType())) {
