...
public class RatingEngineProxy extends MicroserviceRestApiProxy implements ProxyInterface {
    ...
    public List<ColumnMetadata> getIterationMetadata(String customerSpace, String ratingEngineId, String ratingModelId,
        ...
        return JsonUtils.convertList(get("get metadata for a rating model", url, List.class), ColumnMetadata.class);
    }
    ...
    public Map<String, StatsCube> getIterationMetadataCube(String customerSpace, String ratingEngineId,
        ...
        String url = constructUrl(URL_PREFIX + "/{ratingEngineId}/ratingmodels/{ratingModelId}/metadata/cube",
                shortenCustomerSpace(customerSpace), ratingEngineId, ratingModelId)
                + (StringUtils.isNotEmpty(dataStores) ? "?data_stores=" + dataStores : "");
        return JsonUtils.convertMap(get("get metadata cube for a rating model", url, Map.class), String.class,
                StatsCube.class);
    }

    public TopNTree getIterationMetadataTopN(String customerSpace, String ratingEngineId, String ratingModelId,
            String dataStores) {
        String url = constructUrl(URL_PREFIX + "/{ratingEngineId}/ratingmodels/{ratingModelId}/metadata/topn",
                shortenCustomerSpace(customerSpace), ratingEngineId, ratingModelId)
                + (StringUtils.isNotEmpty(dataStores) ? "?data_stores=" + dataStores : "");
        return JsonUtils.convertValue(get("get metadata TopNTree for a rating model", url, Map.class), TopNTree.class);
    }
    ...
    public String modelRatingEngine(String customerSpace, String ratingEngineId, String ratingModelId,
            List<ColumnMetadata> attributes, String userEmail, boolean skipValidation) {
        String url = constructUrl(URL_PREFIX
                + "/{ratingEngineId}/ratingmodels/{ratingModelId}/model?useremail={userEmail}&skip-validation={skipValidation}",
                shortenCustomerSpace(customerSpace), ratingEngineId, ratingModelId, userEmail, skipValidation);
    ...
    public List<RatingModel> getAllModels(String customerSpace) {
        String url = constructUrl(URL_PREFIX + "/allmodels", shortenCustomerSpace(customerSpace));
        return getList("get all models in customer space", url, RatingModel.class);
    }
