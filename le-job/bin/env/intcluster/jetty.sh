...
JAVA_OPTIONS=(-XX:MaxPermSize=2048m -Dsqoop.throwOnError=true -Djetty.class.path=$JAVA_HOME/lib/tools.jar:$HADOOP_HOME/hadoop-common.jar:/etc/hadoop/conf.empty -DJOB_PROPDIR=/etc/ledp -DDATAPLATFORM_PROPDIR=/etc/ledp -DDATAFLOW_PROPDIR=/etc/ledp -DPROPDATA_MADISON_QUARTZ_ENABLED=enabled -DPROPDATA_PROPDIR=/etc/ledp -DDPDL_QUARTZ_ENABLED=disabled)
