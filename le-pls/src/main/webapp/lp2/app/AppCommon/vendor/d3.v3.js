d3 = function() {
  ...
  var pi = Math.PI, tau = 2 * pi, halfpi = pi / 2, epsilon = 1e-6, epsilon2 = epsilon * epsilon, d3_radians = pi / 180, d3_degrees = 180 / pi;
  function d3_sgn(x) {
  ...
  d3.interpolateZoom = function(p0, p1) {
    ...
    var dx = ux1 - ux0, dy = uy1 - uy0, d2 = dx * dx + dy * dy, d1 = Math.sqrt(d2), b0 = (w1 * w1 - w0 * w0 + rho4 * d2) / (2 * w0 * rho2 * d1), b1 = (w1 * w1 - w0 * w0 - rho4 * d2) / (2 * w1 * rho2 * d1), r0 = Math.log(Math.sqrt(b0 * b0 + 1) - b0), r1 = Math.log(Math.sqrt(b1 * b1 + 1) - b1), dr = r1 - r0, S = (dr || Math.log(w1 / w0)) / rho;
    function interpolate(t) {
      var s = t * S;
      if (dr) {
        var coshr0 = d3_cosh(r0), u = w0 / (rho2 * d1) * (coshr0 * d3_tanh(rho * s + r0) - d3_sinh(r0));
        return [ ux0 + u * dx, uy0 + u * dy, w0 * coshr0 / d3_cosh(rho * s + r0) ];
      }
      return [ ux0 + t * dx, uy0 + t * dy, w0 * Math.exp(rho * s) ];
    }
  ...
  function d3_geo_areaRingStart() {
    ...
    function nextPoint(lambda, phi) {
      ...
      var dlambda = lambda - lambda0, cosphi = Math.cos(phi), sinphi = Math.sin(phi), k = sinphi0 * sinphi, u = cosphi0 * cosphi + k * Math.cos(dlambda), v = k * Math.sin(dlambda);
  ...
  function d3_geo_pointInPolygon(point, polygon) {
    ...
    for (var i = 0, n = polygon.length; i < n; ++i) {
      ...
      while (true) {
        if (j === m) j = 0;
        point = ring[j];
        var lambda = point[0], phi = point[1] / 2 + pi / 4, sinphi = Math.sin(phi), cosphi = Math.cos(phi), dlambda = lambda - lambda0, antimeridian = abs(dlambda) > pi, k = sinphi0 * sinphi;
        d3_geo_areaRingSum.add(Math.atan2(k * Math.sin(dlambda), cosphi0 * cosphi + k * Math.cos(dlambda)));
        polarAngle += antimeridian ? dlambda + (dlambda >= 0 ? tau : -tau) : dlambda;
  ...
  function d3_geo_resample(project) {
    ...
    function resampleLineTo(x0, y0, lambda0, a0, b0, c0, x1, y1, lambda1, a1, b1, c1, depth, stream) {
      ...
      if (d2 > 4 * delta2 && depth--) {
        var a = a0 + a1, b = b0 + b1, c = c0 + c1, m = Math.sqrt(a * a + b * b + c * c), phi2 = Math.asin(c /= m), lambda2 = abs(abs(c) - 1) < epsilon ? (lambda0 + lambda1) / 2 : Math.atan2(b, a), p = project(lambda2, phi2), x2 = p[0], y2 = p[1], dx2 = x2 - x0, dy2 = y2 - y0, dz = dy * dx2 - dx * dy2;
  ...
  function d3_geo_projectionRadians(stream) {
    var transform = new d3_geo_transform(stream);
    transform.point = function(lambda, phi) {
  ...
  d3.geo.distance = function(a, b) {
    var capitalDeltalambda = (b[0] - a[0]) * d3_radians, phi0 = a[1] * d3_radians, phi1 = b[1] * d3_radians, sincapitalDeltalambda = Math.sin(capitalDeltalambda), coscapitalDeltalambda = Math.cos(capitalDeltalambda), sinphi0 = Math.sin(phi0), cosphi0 = Math.cos(phi0), sinphi1 = Math.sin(phi1), cosphi1 = Math.cos(phi1), t;
    return Math.atan2(Math.sqrt((t = cosphi1 * sincapitalDeltalambda) * t + (t = cosphi0 * sinphi1 - sinphi0 * cosphi1 * coscapitalDeltalambda) * t), sinphi0 * sinphi1 + cosphi0 * cosphi1 * coscapitalDeltalambda);
  };
  ...
  function d3_geo_lengthLineStart() {
    ...
    function nextPoint(lambda, phi) {
      var sinphi = Math.sin(phi *= d3_radians), cosphi = Math.cos(phi), t = abs((lambda *= d3_radians) - lambda0), coscapitalDeltalambda = Math.cos(t);
      d3_geo_lengthSum += Math.atan2(Math.sqrt((t = cosphi * Math.sin(t)) * t + (t = cosphi0 * sinphi - sinphi0 * cosphi * coscapitalDeltalambda) * t), sinphi0 * sinphi + cosphi0 * cosphi * coscapitalDeltalambda);
  ...
  function d3_geo_conicConformal(phi0, phi1) {
    ...
    function forward(lambda, phi) {
      var rho = abs(abs(phi) - halfpi) < epsilon ? 0 : F / Math.pow(t(phi), n);
  ...
  function d3_geo_transverseMercator(lambda, phi) {
    ...
    return [ Math.log((1 + B) / (1 - B)) / 2, Math.atan2(Math.tan(phi), Math.cos(lambda)) ];
  }
  ...
  d3.layout.pie = function() {
    var value = Number, sort = d3_layout_pieSortByValue, startAngle = 0, endAngle = tau;
    function pie(data) {
  ...
  d3.svg.arc = function() {
    ...
    function arc() {
      ...
      a0 = a1, a1 = da), a1 - a0), df = da < pi ? "0" : "1", c0 = Math.cos(a0), s0 = Math.sin(a0), c1 = Math.cos(a1), s1 = Math.sin(a1);
  ...
  var d3_svg_arcOffset = -halfpi, d3_svg_arcMax = tau - epsilon;
