...
public class ProjectServiceImpl implements ProjectService {
    ...
    public List<ProjectSummary> getAllProjects(String customerSpace, Boolean includeSources, Boolean includeArchived) {
        return projectProxy.getAllDCPProject(customerSpace, includeSources, includeArchived, 0, DEFAULT_PAGE_SIZE, TeamUtils.getTeamIds());
    }

    @Override
    public List<ProjectSummary> getAllProjects(String customerSpace, Boolean includeSources, Boolean includeArchived, int pageIndex, int pageSize) {
        Preconditions.checkArgument(pageIndex > 0);
        Preconditions.checkArgument(pageSize > 0);
        return projectProxy.getAllDCPProject(customerSpace, includeSources, includeArchived, pageIndex - 1, pageSize, TeamUtils.getTeamIds());
    }
