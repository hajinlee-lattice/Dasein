    ...
    public String checkModelDownloader() {
        try {
            if (applicationContext.containsBean("modelSummaryDownloadJob")) {
                ...
                if (timeStamp.getSeconds() - timeStampContainer.getTimeStamp()
                        .getSeconds() <= downloaderInvokeInterval) {
                    long diff = timeStamp.getSeconds()
                            - timeStampContainer.getTimeStamp().getSeconds();
                    return String.format(
                            "[SUCCESS] ModelDownloader job has been running for %d seconds.", diff);
                }
                return String.format(
                        "[FAILURE] ModelDownloaderJob has been running for more than %d seconds.",
