...
import static com.latticeengines.pls.metadata.resolution.MetadataResolver.getCategoryBasedOnSchemaType;
import static com.latticeengines.pls.metadata.resolution.MetadataResolver.isBooleanTypeColumn;
...
public class MetadataResolverUnitTestNG {
    ...
    public void testGetCategoryBasedOnSchemaType() {
        Assert.assertEquals(getCategoryBasedOnSchemaType("SalesforceAccount"),
                ModelingMetadata.CATEGORY_ACCOUNT_INFORMATION);
        Assert.assertEquals(getCategoryBasedOnSchemaType("SalesforceLead"),
                ModelingMetadata.CATEGORY_LEAD_INFORMATION);
        Assert.assertEquals(getCategoryBasedOnSchemaType("Category"),
                ModelingMetadata.CATEGORY_LEAD_INFORMATION);
        Assert.assertEquals(getCategoryBasedOnSchemaType("someRandomeCategory"),
                ModelingMetadata.CATEGORY_LEAD_INFORMATION);
        Assert.assertEquals(getCategoryBasedOnSchemaType(null),
                ModelingMetadata.CATEGORY_LEAD_INFORMATION);
    }
    ...
    public void testTypeParse() {
        ...
        booleanTest.add("1");
        booleanTest.add("0");
        Assert.assertFalse(isBooleanTypeColumn(booleanTest));
        Assert.assertTrue(isBooleanTypeColumn(Arrays.asList("true", "False", "YeS", "No")));

        // case 1: for unified date format
        MutableTriple<String, String, String> formatForDateAndTime =
                distinguishDateAndTime(Arrays.asList("11/4/2016", "12/05/2018"));
        ...
        Assert.assertNull(distinguishDateAndTime(Arrays.asList("11/41/2016", "12/q@ax/2018", "1q2/q@ax/2019")));

        // case 3: 55 not in appropriate range of month and date, expected false
        Assert.assertNull(distinguishDateAndTime(Arrays.asList("11/55/2016", "12/55/2018", "42/05/2018")));

        // case 4: M-d-yyyy and M-d-yyyy have same occurrence times, hit M/d/yyyy => MM/DD/YYYY.
        formatForDateAndTime = distinguishDateAndTime(
        ...
        Assert.assertNotNull(formatForDateAndTime = distinguishDateAndTime(Arrays.asList("11/4/2016",
                "12/05/2018")));
        Assert.assertEquals(formatForDateAndTime.getLeft(), "MM/DD/YYYY");
        Assert.assertNull(formatForDateAndTime.getMiddle());
        Assert.assertNull(formatForDateAndTime.getRight());
        ...
        Assert.assertNotNull(formatForDateAndTime = distinguishDateAndTime(
        ...
        Assert.assertNotNull(formatForDateAndTime = distinguishDateAndTime(Arrays.asList("11/4/2016 4:20:10"
                , "12/05/2018 20:10:20")));
        Assert.assertEquals(formatForDateAndTime.getLeft(), "MM/DD/YYYY");
        ...
        // Bug PLS-14490
        Assert.assertNull(formatForDateAndTime = distinguishDateAndTime(new ArrayList<>()));
    }
}
