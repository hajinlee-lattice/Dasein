...
public class AttributeResourceDeploymentTestNG extends PlsDeploymentTestNGBase {
    ...
    public void setup() throws NoSuchAlgorithmException, KeyManagementException, IOException {
        ...
        List<?> raw = testBed.getRestTemplate().getForObject(getRestAPIHostPort() + "/pls/latticeinsights/insights",
                List.class);
    ...
    public void testRetrieveAfterSaveMultiple() {
        String url = getRestAPIHostPort()
                + String.format("/pls/attributes/flags/%s/CompanyProfile/%s", attributeName, "hidden");
        String retrieved = testBed.getRestTemplate().getForObject(url, String.class);
        assertNotNull(retrieved);
