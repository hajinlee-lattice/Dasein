...
public class RatingEngineNoteResourceDeploymentTestNG extends PlsDeploymentTestNGBase {
    ...
    public void testCreate() {
        ...
        Assert.assertTrue(restTemplate.postForObject(
                getRestAPIHostPort() + "/pls/ratingenginenotes/" + ratingEngine.getId(), noteParams, Boolean.class));
        List<?> listObject = restTemplate
                .getForObject(getRestAPIHostPort() + "/pls/ratingenginenotes/" + ratingEngine.getId(), List.class);
    ...
    public void testUpdate() {
        ...
        Assert.assertTrue(restTemplate.postForObject(
                getRestAPIHostPort() + "/pls/ratingenginenotes/" + ratingEngine.getId() + "/" + note1.getId(),
    ...
    public void testDelete() {
        restTemplate
                .delete(getRestAPIHostPort() + "/pls/ratingenginenotes/" + ratingEngine.getId() + "/" + note1.getId());
        List<?> listObject = restTemplate
                .getForObject(getRestAPIHostPort() + "/pls/ratingenginenotes/" + ratingEngine.getId(), List.class);
        List<RatingEngineNote> noteList = JsonUtils.convertList(listObject, RatingEngineNote.class);
        Assert.assertNotNull(noteList);
