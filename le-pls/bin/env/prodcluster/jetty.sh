...
JAVA_OPTIONS=(-XX:MaxPermSize=2048m -Dsqoop.throwOnError=true -Dcom.latticeengines.registerBootstrappers=true -Djetty.class.path=$JAVA_HOME/lib/tools.jar:$HADOOP_HOME/hadoop-common.jar:/etc/hadoop/conf.empty -DDB_PROPDIR=/etc/ledp -DPLS_PROPDIR=/etc/ledp -DSECURITY_PROPDIR=/etc/ledp)
