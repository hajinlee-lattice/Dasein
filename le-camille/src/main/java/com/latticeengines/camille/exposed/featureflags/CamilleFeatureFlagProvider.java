...
public class CamilleFeatureFlagProvider implements FeatureFlagProvider {
	...
	public CamilleFeatureFlagProvider() {
		definitionCache = ConfigurationCache.construct(new PodScope(),
	...
	public boolean isEnabled(CustomerSpace space, String id) {
		...
		if (flags == null) {
			log.warn(String.format(
					"No feature flag value file defined for customer space %s",
					space));
			return false;
		}
	...
	public FeatureFlagDefinition getDefinition(String id) {
		...
		if (definitions == null) {
			log.warn(String
					.format("When requesting feature flag %s, no feature flag definition file found",
							id));
			return null;
		}
		...
		if (definition == null) {
			log.warn(String
					.format("Definition missing for feature flag %s", id));
		}
	...
	@Override
	public void setEnabled(final CustomerSpace space, final String id,
			final boolean enabled) {
		FeatureFlagDefinition featureFlag = getDefinition(id);
		if (featureFlag == null) {
			throw new RuntimeException(
					String.format(
							"Feature flag %s cannot be toggled without a corresponding definition",
							id));
		}

		if (!featureFlag.getConfigurable()) {
			throw new RuntimeException(
					String.format(
							"Feature flag %s cannot be toggled because it is not configurable",
							id));
		}

		SafeUpserter upserter = new SafeUpserter();
		upserter.upsert(new CustomerSpaceScope(space), new Path("/"
				+ PathConstants.FEATURE_FLAGS_VALUES_FILE),
				new Function<FeatureFlagValueMap, FeatureFlagValueMap>() {
					@Override
					public FeatureFlagValueMap apply(
							FeatureFlagValueMap existing) {
	...
	@Override
	public void setDefinition(final String id,
		...
		upserter.upsert(
				...
				new Function<FeatureFlagDefinitionMap, FeatureFlagDefinitionMap>() {
					@Override
					public FeatureFlagDefinitionMap apply(
							FeatureFlagDefinitionMap existing) {
	...
	private void rebuildValues(CustomerSpace space) {
		try {
			valueCache.rebuild(new CustomerSpaceScope(space), new Path("/"
					+ PathConstants.FEATURE_FLAGS_VALUES_FILE));
		} catch (Exception e) {
	...
	public FeatureFlagDefinitionMap getDefinitions() {
		...
		try {
			...
			log.warn(
					"Exception occurred attempting to retrieve feature flag definitions",
					e);
			doc = null;
		}

		if (doc != null) {
			return DocumentUtils.toTypesafeDocument(doc,
					FeatureFlagDefinitionMap.class);
		} else {
	...
	public FeatureFlagValueMap getFlags(CustomerSpace space) {
		...
		try {
			...
			log.warn("Exception occurred attempting to retrieve feature flags",
					e);
			toReturn = null;
		}
		if (toReturn == null) {
