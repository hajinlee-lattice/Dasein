...
public class FeatureFlagUnitTestNG {
	...
	public void testEnableFlag() throws Exception {
		...
		Assert.assertFalse(FeatureFlagClient.isEnabled(
				CamilleTestEnvironment.getCustomerSpace(), id1));
		...
		try {
			FeatureFlagClient.setEnabled(
					CamilleTestEnvironment.getCustomerSpace(), id1, true);
			Assert.fail("Should have thrown RuntimeException.");
		} catch (Exception e) {
			Assert.assertTrue(true,
					"Feature flag TestFlag cannot be toggled because it is not configurable");
		}

		final String id2 = "TestFlag2";
		Assert.assertFalse(FeatureFlagClient.isEnabled(
				CamilleTestEnvironment.getCustomerSpace(), id2));
		...
		try {
			FeatureFlagClient.setEnabled(
					CamilleTestEnvironment.getCustomerSpace(), id2, true);
			Assert.assertTrue(FeatureFlagClient.isEnabled(
					CamilleTestEnvironment.getCustomerSpace(), id2));
		} catch (Exception e) {
