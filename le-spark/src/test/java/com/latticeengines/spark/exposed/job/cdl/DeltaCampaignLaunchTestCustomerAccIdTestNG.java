...
public class DeltaCampaignLaunchTestCustomerAccIdTestNG extends TestJoinTestNGBase {

    private static final Logger log = LoggerFactory.getLogger(DeltaCampaignLaunchTestCustomerAccIdTestNG.class);
    ...
    public void verifyResult(SparkJobResult result) {
        ...
        Assert.assertEquals(addCsvDf.getCount().intValue(), (addAccounts.length - 1) * completeContactPerAccount + 1);
    ...
    private DeltaCampaignLaunchSparkContext generateDeltaCampaignLaunchSparkContext(boolean launchToDb) {
        ...
        if (launchToDb) {
            playLaunch.setDestinationSysName(CDLExternalSystemName.Eloqua);
        } else {
