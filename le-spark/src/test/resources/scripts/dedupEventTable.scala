...
val eventCol =  lattice.params.get("EVENT")
...
val dfFalseEvent = dfEventTableFilter.filter($"Event"===false).withColumn(sort_event, lit(0))
val dfTrueEvent = dfEventTableFilter.filter($"Event"===true).withColumn(sort_event, lit(1))
...
val dfHasDedupeId = dfMergedEventOutput.filter($"__LDC_DedupeId__".isNotNull)
val dfNoDedupeId = dfMergedEventOutput.filter($"__LDC_DedupeId__".isNull)
// dedup based on : group by INT_LDC_DEDUPE_ID column and sory by sortevent column in desc order
val window_DedupeIdGroupSortDescCol = Window.partitionBy($"__LDC_DedupeId__").orderBy($"__Sort_Event__".desc)
