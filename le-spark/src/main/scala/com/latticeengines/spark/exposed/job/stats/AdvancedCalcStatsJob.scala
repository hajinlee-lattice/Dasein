...
// support rollup and dedup --- may not be fully working yet
class AdvancedCalcStatsJob extends AbstractSparkJob[AdvancedCalcStatsConfig] {
  ...
  override def runJob(spark: SparkSession, lattice: LatticeContext[AdvancedCalcStatsConfig]): Unit = {
    val inputData: DataFrame = lattice.input.head
    val profileData: DataFrame = lattice.input(1)
    val config: AdvancedCalcStatsConfig = lattice.config
  ...
  private def expandEncodedByAM(input: DataFrame, cols: Seq[String], config: AdvancedCalcStatsConfig): DataFrame = {
