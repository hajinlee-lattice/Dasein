...
class CalcStatsJob extends AbstractSparkJob[CalcStatsConfig] {
  ...
  private def toBktId(spark: SparkSession, depivoted: DataFrame, profile: DataFrame): DataFrame = {
    val algoMap = spark.sparkContext.broadcast(profile.collect().map(row => {
    ...
    val bktIdFunc = spark.sparkContext.broadcast((attrName: String, value: Any) => if (value == null) {
      0
    } else if (algoMap.value.contains(attrName)) {
      val algo = algoMap.value(attrName)
      algo match {
        case bkt: DiscreteBucket =>
