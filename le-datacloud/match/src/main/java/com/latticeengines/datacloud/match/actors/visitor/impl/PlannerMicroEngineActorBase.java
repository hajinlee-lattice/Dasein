...
public abstract class PlannerMicroEngineActorBase extends ExecutorMicroEngineTemplate {
    ...
    protected boolean validateMatchFields(@NotNull MatchTraveler traveler) {
        ...
        res &= validateMatchField(MatchKey.NationalID, tuple.getNationalID(), record);
        ...
        res &= validateMatchField(MatchKey.NationalID, tuple.getStreetAddress1(), record);
        if (record != null && MapUtils.isNotEmpty(record.getParsedPreferredEntityIds())) {
            ...
            if (!isValid) {
                record.getParsedPreferredEntityIds().put(traveler.getEntity(), null);
            }
            res &= isValid;
        }
