...
public class PivotFlow extends ConfigurableFlowBase<PivotConfig> {
    ...
    public Node construct(TransformationFlowParameters parameters) {
        PivotConfig config = getTransformerConfig(parameters);
        ...
        Node join = joinedConfigurablePipes(parameters, sourceMap, config);
        ...
        if (config.hasSqlPresence()) {
            join = FlowUtils.truncateStringFields(join, parameters.getColumns());
        }
        return join;
    }

    protected Node joinedConfigurablePipes(TransformationFlowParameters parameters, Map<String, Node> sourceMap,
            PivotConfig config) {
        Node pivot = joinedPivotedPipes(parameters, sourceMap, config);
        Node agg = joinedAggregatedPipes(parameters, sourceMap, config);
    ...
    private Node joinedPivotedPipes(TransformationFlowParameters parameters, Map<String, Node> sourceMap,
            PivotConfig config) {
        List<SourceColumn> columns = parameters.getColumns();
        String[] joinFields = config.getJoinFields();
    ...
    private Node joinedAggregatedPipes(TransformationFlowParameters parameters, Map<String, Node> sourceMap,
            PivotConfig config) {
        List<SourceColumn> columns = parameters.getColumns();
        String[] joinFields = config.getJoinFields();
