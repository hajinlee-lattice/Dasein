...
public class PropDataEntityMgrImplTestNG extends
	...
	public void createCommands() throws Exception {
		Commands commands = PropDataTestUtils.createNewCommands("Docusign",
				"Match_source_ForPropDataMatching999");
		...
		Commands actualCommands = propDataEntityMgr.getCommands(commands
				.getPid());
	...
	private void assertCommandIds(CommandIds commandIds,
			CommandIds actualCommandIds) {
		Assert.assertEquals(actualCommandIds.getPid(), commandIds.getPid());
		Assert.assertEquals(actualCommandIds.getCreatedBy(),
				commandIds.getCreatedBy());
	...
	private void assertCommands(Commands commands, Commands actualCommands) {
		...
		Assert.assertEquals(actualCommands.getCommandName(),
				commands.getCommandName());
		Assert.assertEquals(actualCommands.getContractExternalID(),
				commands.getContractExternalID());
		Assert.assertEquals(actualCommands.getDeploymentExternalID(),
				commands.getDeploymentExternalID());
		Assert.assertEquals(actualCommands.getDestTables(),
				commands.getDestTables());
		Assert.assertEquals(actualCommands.getProcessUID(),
				commands.getProcessUID());
		Assert.assertEquals(actualCommands.getProfileID(),
				commands.getProfileID());
		Assert.assertEquals(actualCommands.getSourceTable(),
				commands.getSourceTable());
...
//				commands.getCommandStatus());
		...
		Assert.assertEquals(actualCommands.getIsDownloading(),
				commands.getIsDownloading());
		Assert.assertEquals(actualCommands.getMaxNumRetries(),
				commands.getMaxNumRetries());
		Assert.assertEquals(actualCommands.getNumRetries(),
				commands.getNumRetries());
