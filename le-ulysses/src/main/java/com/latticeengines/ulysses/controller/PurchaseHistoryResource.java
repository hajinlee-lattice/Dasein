...
public class PurchaseHistoryResource {
    ...
    // Spend Analytics always uses Natural Calendar with a start date of Jan 1st 2000
    private final LocalDate DEFAULT_SPEND_ANALYTICS_START_DATE = LocalDate.of(DEFAULT_START_YEAR, 1,
            1);
    ...
    public FrontEndResponse<List<String>> getPurchaseHistoryAccountById(
            RequestEntity<String> requestEntity, @PathVariable String crmAccountId) {
        String customerSpace = CustomerSpace.parse(MultiTenantContext.getTenant().getId())
                .toString();
        try {
            DataPage accountData = dataLakeService.getAccountById(crmAccountId,
...
<<<<<<< Updated upstream
                ...
                List<PeriodTransaction> periodTransactions = periodTransactionProxy
                        .getPeriodTransactionsByAccountId(customerSpace, accountId,
                                defaultPeriodName, ProductType.Spending);
...
>>>>>>> Stashed changes
    ...
    public FrontEndResponse<List<String>> getPurchaseHistoryAccountBySegment(
        ...
        Map<String, ColumnMetadata> attributeMap = Flux
                .fromIterable(dataLakeService.getCachedServingMetadataForEntity(customerSpace,
        ...
        if (!attributeMap.containsKey(InterfaceName.SpendAnalyticsSegment.name())) {
            log.error(InterfaceName.SpendAnalyticsSegment.name() + " does not exist for tenant: "
                    + customerSpace);
