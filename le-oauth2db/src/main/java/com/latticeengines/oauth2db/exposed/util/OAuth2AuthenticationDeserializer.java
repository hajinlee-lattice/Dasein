...
public class OAuth2AuthenticationDeserializer extends JsonDeserializer<OAuth2Authentication> {
    ...
    /*
     * public static UserDetails jsonToUserDetails(JsonNode json) { if (json == null
     * || json.getNodeType() == JsonNodeType.NULL) { return null; } ObjectNode node
     * = (ObjectNode) json; UserDetails userDetails = new User(getText(node,
     * STRING_USERNAME), getText(node, STRING_PASSWORD),
     * node.get(STRING_ENABLED).booleanValue(),
     * node.get(STRING_ACCOUNT_NON_EXPIRED).booleanValue(),
     * node.get(STRING_CREDENTIALS_NON_EXPIRED).booleanValue(),
     * node.get(STRING_ACCOUNT_NON_LOCKED).booleanValue(),
     * arrayToGrantedAuthorities(node.get(STRING_AUTHORITIES))); return userDetails;
     * }
     */
}
