...
def main(argv):
    ...
    if parsedResult.env not in envDic.keys():
        print("Invalid -env flag. Valid values:")
        pprint.pprint(envDic)
        sys.tracebacklimit=0
        raise ValueError('The parsedResult does not name a valid environment.'+
                         os.linesep+'Use -h to see more options.')
    else:
...
def updateDiagnosticFlag(contract, env, newFlags, zk):
    ...
    if (zk.exists(znode)):
        ...
        print("Doing nothing. Could not find", znode)
...
def parseArguments():
    ...
    tenantgroup = parser.add_mutually_exclusive_group()
    tenantgroup.add_argument("-tenant", help="Name of the Tenant to update. This takes precedence over a -f file.")
    tenantgroup.add_argument("-f", help='Specify a file listing Tenants to modify with one Tenant name per line.')
    tenantgroup.add_argument("-all", "--allTenants", action="store_true", help="Apply to all the tenants in the environment")
