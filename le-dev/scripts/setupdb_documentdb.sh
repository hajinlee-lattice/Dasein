...
DDL="$WSHOME/ddl_documentdb_mysql5innodb.sql"
if [ ! -f "${DDL}" ]; then
...
if version_gt ${mysql_version} ${threshold_version}; then
    ...
    sed "s|WSHOME|$WSHOME|g" $WSHOME/le-dev/scripts/setupdb_documentdb.sql | sed "s|LOAD DATA INFILE|LOAD DATA LOCAL INFILE|g" | eval $MYSQL_COMMAND
else
    echo "MySQL version $mysql_version"
    sed "s|WSHOME|$WSHOME|g" $WSHOME/le-dev/scripts/setupdb_documentdb.sql | eval $MYSQL_COMMAND
fi
