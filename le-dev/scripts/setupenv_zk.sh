...
if [ "${LE_USING_DOCKER}" != "true" ] && [ "${BOOTSTRAP_MODE}" = "bootstrap" ]; then
    ...
    if [ ! -f "$ARTIFACT_DIR/zookeeper-3.4.13.tar.gz" ]; then
        wget http://apache.claz.org/zookeeper/zookeeper-3.4.13/zookeeper-3.4.13.tar.gz -O $ARTIFACT_DIR/zookeeper-3.4.13.tar.gz
    fi

    tar xzf $ARTIFACT_DIR/zookeeper-3.4.13.tar.gz -C $ARTIFACT_DIR
    cp -r $ARTIFACT_DIR/zookeeper-3.4.13/* $ZOOKEEPER_HOME
    rm -rf $ARTIFACT_DIR/zookeeper-3.4.13
