...
if [[ "${UNAME}" == 'Darwin' ]]; then
    ...
    sed -i '' 's/alter table .* drop foreign key .*;//g' ddl_pls_multitenant_mysql5innodb.sql
else
...
if version_gt ${mysql_version} ${threshold_version}; then
    ...
    sed "s|WSHOME|$WSHOME|g" setupdb_pls_multitenant_migration.sql | sed "s|LOAD DATA INFILE|LOAD DATA LOCAL INFILE|g" | eval $MYSQL_COMMAND

else
    echo "MySQL version $mysql_version"
    sed "s|WSHOME|$WSHOME|g" setupdb_pls_multitenant_migration.sql | eval $MYSQL_COMMAND
fi
