...
if version_gt ${mysql_version} ${threshold_version}; then
    ...
    sed "s|WSHOME|$WSHOME|g" setupdb_oauth2_migration.sql | sed "s|LOAD DATA INFILE|LOAD DATA LOCAL INFILE|g" | eval $MYSQL_COMMAND

else
    echo "MySQL version $mysql_version"
    sed "s|WSHOME|$WSHOME|g" setupdb_oauth2_migration.sql | eval $MYSQL_COMMAND
fi
