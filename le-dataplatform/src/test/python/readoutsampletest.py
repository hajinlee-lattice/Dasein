...
class ReadoutSampleTest(TrainingTestBase):
    ...
    def checkResults(self, expectedRows):
        ...
        for i in range(rows):
            value = dataFrame.iget_value(i, scoreColumnIndex)
            self.assertLessEqual(value, 1)
            self.assertGreaterEqual(value, 0)

        # Converted Range as Expected?
        for i in range(rows):
            value = dataFrame.iget_value(i, convertedColumIndex)
        ...
        for i in range(rows):
            copyValue = dataFrameCopy.iget_value(i, scoreColumnIndex)
            value = dataFrame.iget_value(i, scoreColumnIndex)
            self.assertEqual(copyValue, value)
