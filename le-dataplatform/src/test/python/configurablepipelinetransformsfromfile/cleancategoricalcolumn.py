...
class CleanCategoricalColumn(PipelineStep):
    ...
    def transform(self, dataFrame, threshold = 0.8, targetColumn=None):
        try:
            ...
            logger.info(self.columnsToConvert)
            for column, encoder in self.columnsToConvert.iteritems():
                if column in dataFrame.columns:
                    dataFrameAsList = [v for v in dataFrame[column].values]
                    logger.info(str( column ))
                    logger.info(str( type(dataFrame[column].values)))
                    logger.info(str( len(dataFrameAsList)))
    ...
    def createSet(self, ccKeys, includedKeys):
        return set(ccKeys)-set(includedKeys)

    def createDict(self, d):
        newDict=dict((k,k) for k in d)
        return newDict

    def createDictNull(self, d, nullValue):
        newDict1=dict((k,nullValue) for k in d)
        return newDict1

    def cleanCateg(self, xlist,thresh=.95,percMin=.01,nullValue='EMPTY'):
        cc=Counter(xlist)
        total=len(xlist)*thresh
        cd=sorted(cc.items(), key = itemgetter(1), reverse=True)
        newtotal=0
        includedKeys=[]
        threshCount=len(xlist)*percMin
        for cnt in xrange(len(cd)):
            xcount=cd[cnt][1]
