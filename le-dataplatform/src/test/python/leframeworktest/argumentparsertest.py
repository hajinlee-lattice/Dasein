...
class ArgumentParserTest(TestBase):
    ...
    def testCreateListForAvro(self):
        ...
        self.assertEqual(training.shape, (17040, 110), "Dimensions of training data should be (17040, 110), but is " + str(training.shape))
        test = parser.createList(parser.stripPath(schema["test_data"]))
        self.assertEqual(test.shape, (4294, 110), "Dimensions of test data should be (4294, 110), but is " + str(test.shape))
