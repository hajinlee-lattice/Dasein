...
/**
 * A RecordReader that reads records from a SQL table, using data-driven WHERE
 * clause splits. Emits LongWritables containing the record number as key and
...
public class DataDrivenDBRecordReader<T extends DBWritable> extends DBRecordReader<T> {
    ...
    protected String getSelectQuery() {
        ...
        return getSelectQuery(dataSplit.getLowerClause(), dataSplit.getUpperClause());
    }

    /**
     * Returns the query for selecting the records, with lower and upper clause
     * consitions provided as parameters This is needed for recovering from
     * connection failures after some data in the split have been already
     * processed
     */
    protected String getSelectQuery(String lowerClause, String upperClause) {
        ...
        if (dbConf.getInputQuery() == null) {
            ...
            if (!dbProductName.startsWith("ORACLE") && !dbProductName.startsWith("DB2")
                    && !dbProductName.startsWith("MICROSOFT SQL SERVER") && !dbProductName.startsWith("POSTGRESQL")) {
                // The AS clause is required for hsqldb. Some other databases
                // might have
