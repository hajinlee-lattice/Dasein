package com.latticeengines.dataplatform.runtime.jetty;
...
public class JettyAppMaster extends StaticEventingAppmaster implements ContainerLauncherInterceptor {

    private final static Log log = LogFactory.getLog(JettyAppMaster.class);
    ...
    private String jettyContainerId;
    ...
    private AtomicInteger port = new AtomicInteger(9031);
    ...
    public ContainerLaunchContext preLaunch(Container container, ContainerLaunchContext context) {
        jettyContainerId = container.getId().toString();
        log.info("Container id = " + jettyContainerId);
        // Start monitoring process
        monitor.start();
        log.info(context.getCommands().get(0));
        String command = context.getCommands().get(0).replace(".jar", ".jar --port " + port);
        port.addAndGet(1);
        context.setCommands(Arrays.<String> asList(command));
        log.info(context.getCommands().get(0));
    ...
    protected boolean onContainerFailed(ContainerStatus status) {
        ...
        if (status.getExitStatus() == ContainerExitStatus.ABORTED) {
            if (!containerId.equals(jettyContainerId)) {
    ...
    private void setRuntimeConfig(Properties parameters) {
        ...
        String configPath = hdfsJobBaseDir + "/" + parameters.getProperty(ContainerProperty.JOBDIR.name()) + "/" + parameters.getProperty(ContainerProperty.RUNTIME_CONFIG.name());
