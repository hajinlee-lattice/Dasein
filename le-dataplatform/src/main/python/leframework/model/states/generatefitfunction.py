...
class FitFunctionGenerator(State):
    ...
    def execute(self):
        ...
        try:
            ...
            gamma, alpha, beta = self.rate_chart_fit((decileRateDf['rate']), avgRate)
    ...
    def rate_chart_fit(self, rateArray, avgRate):
        rateArray = np.asarray(rateArray)

        if rateArray[0] < avgRate:
            return (0.0, 0.0, np.mean(np.log(rateArray + 1e-6)))

        liftArray = rateArray / avgRate
        decile = np.arange(1, liftArray.shape[0] + 1)
