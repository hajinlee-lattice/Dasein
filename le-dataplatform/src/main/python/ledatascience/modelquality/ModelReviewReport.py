# -*- coding: utf-8 -*-
"""
Created on Thu Jan 26 10:21:26 2017
...
def printOutResults(data, cols, outputFile, sheetNum, sheetName, set_width=False):
    if sheetNum == 1:
        wb = Workbook()
        ws1 = wb.active
        ws1.title = sheetName
...
def loadEventtable(train, test):
    ...
    if os.path.isfile(train):
        colNames_1, data_1 = loadCSVfile(train)
    else:
    ...
    if os.path.isfile(test):
        if '.csv' in test:
            colNames_2, data_2 =  loadCSVfile(test)
        else:
            colNames_2, data_2 =  readInAvro(test)
...
def modelReviewReport(): 
    ...
    tp_col, tp_data = loadCSVfile(param.topPredictor_file)
    
    aucScore, liftScore, rate, colNameList = loadJsonfile(param.modelsummaryJson_file)
    
    rf_col,rf_data = loadCSVfile(param.rfmodel_file)
    
    ex_col, ex_data = loadCSVfile(param.expectFeat_file)
    
    lim_col, lim_data = loadCSVfile(param.expectFeatLimit_file)
    
    evtTable_data, evtTable_col = loadEventtable(param.trainDataFile,param.testDataFile)

    countryGroup_col, countryGroup_data = loadCSVfile(param.countryGrouping_file)

    evtTable_data = [x + [None] for x in evtTable_data]
    checkColumnExistence(evtTable_data, evtTable_col, param)
    #score_col, score_data = loadCSVfile(param.scoreFile)
    ...
    printOutResults(sampleSplitting, 
                    ['Criteria',  'Group', 'Lift',
...
#    f_smallpophighpred = [fname in smallpred_features for fname in fnames] 
    ...
    printOutResults(feature_removals,
    ...
    printOutResults(feature_warnings,
    ...
    printOutResults(posPredsmallPop, 
    ...
    printOutResults(internalFeatReport, 
