...
class ImputationStep(PipelineStep):
    ...
    def generateTransformedBooleanColumns(self, dataFrame, calculateImputationValues):
        ...
        if len(nullValues) > 0:
            ...
            nullCol_train_transformed = self.getTransformedMatrix(self.scalingArray, self.meanColumn, self.componentMatrix, nullCols)
            if len(nullCol_train_transformed > 0):
                nullCol_train_transformed = pd.DataFrame(nullCol_train_transformed)
                nullCol_train_transformed.columns = ["Transformed_Boolean_"+str(nullCol_train_transformed.columns.values[i]) for i in range(len(nullCol_train_transformed.columns.values))]
                outputFrame = pd.concat([outputFrame, nullCol_train_transformed], axis = 1)
        del nullCols
