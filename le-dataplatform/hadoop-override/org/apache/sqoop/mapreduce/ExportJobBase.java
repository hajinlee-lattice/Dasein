...
public class ExportJobBase extends JobBase {
    ...
    /**
     * What column of the table to use for the WHERE clause of an updating
     * export.
     */
    ...
    @Override
    protected void configureMapper(Job job, String tableName, String tableClassName) throws ClassNotFoundException,
            IOException {
    ...
    protected boolean runJob(Job job) throws ClassNotFoundException, IOException, InterruptedException {
        ...
        if (null == jobCounters) {
            ...
            perfCounters.addBytes(jobCounters.getGroup("FileSystemCounters").findCounter("HDFS_BYTES_READ").getValue());
     ...
     *             if the job fails unexpectedly or is misconfigured.
     */
    public void runExport() throws ExportException, IOException {
        ...
        if (stagingTableName != null) { // user has specified the staging table
            if (cmgr.supportsStagingForExport()) {
                ...
                throw new ExportException("The active connection manager (" + cmgr.getClass().getCanonicalName()
        ...
        if (stagingEnabled) {
            ...
            if (options.doClearStagingTable()) {
                ...
                // User has not explicitly specified the clear staging table
                // option.
                // Assert that the staging table is empty.
                try {
                    long rowCount = cmgr.getTableRowCount(stagingTableName);
                    if (rowCount != 0L) {
                        throw new ExportException("The specified staging table (" + stagingTableName
                                + ") is not empty. To force deletion of "
        ...
        if (stagingEnabled) {
            ...
            try {
                ...
                LoggingUtils.logAll(LOG, "Failed to move data from staging table (" + stagingTableName
                        + ") to target table (" + outputTableName + ")", ex);
    ...
    protected void validateExport(String tableName, Configuration conf, Job job) throws ExportException {
        ...
        try {
            ValidationContext validationContext = new ValidationContext(getRowCountFromHadoop(job), getRowCountFromDB(
                    context.getConnManager(), tableName));
    ...
    protected boolean inputIsSequenceFiles() {
        try {
            return isSequenceFiles(context.getOptions().getConf(), getInputPath());
        } catch (IOException ioe) {
