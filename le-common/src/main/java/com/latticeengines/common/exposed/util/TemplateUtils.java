...
public final class TemplateUtils {
    ...
    private static Configuration cfgWithIterableSupport;

    public static String renderByJson(String template, JsonNode params) {
        return render(template, params, getCfg());
    }
    ...
    public static String renderByMap(String template, Map<String, Object> params, boolean enableIterableSupport) {
        return render(template, params, enableIterableSupport ? getCfgWithIterableSupport() : getCfg());
    }
    ...
    private static String render(String template, Object params, Configuration cfg) {
        Template tpl;
        try {
            tpl = new Template("inline-template", template, cfg);
        } catch (IOException e) {
    ...
    private static Configuration getCfgWithIterableSupport() {
        if (cfgWithIterableSupport == null) {
            configure();
        }
        return cfgWithIterableSupport;
    }

    private static synchronized void configure() {
        if (cfg == null) {
            TemplateUtils.cfg = newCfg();
        }
        if (cfgWithIterableSupport == null) {
            Configuration configuration = newCfg();
            ...
            cfgWithIterableSupport = configuration;
        }
    }

    private static Configuration newCfg() {
