...
def create_template():
    ...
    asgroup, launchconfig = create_bkr_asgroup(ecscluster, instanceprofile, [ elb9092 ])
    ...
    sr_rsrcs = sr_resources(instanceprofile, bkr, elb9092)
    elb9022, elb9023, elb9024 = sr_rsrcs[:3]
...
def create_bkr_asgroup(ecscluster, instanceprofile, elbs):
    asgroup = AutoScalingGroup("BrokerScalingGroup")
    launchconfig = LaunchConfiguration("BrokerContainerPool", instanceprofile, instance_type_ref="BrokerInstanceType")
...
def sr_resources(instanceprofile, bkr_service, elb9092):
    ...
    asgroup, launchconfig = create_sr_asgroup(ecscluster, instanceprofile, [elb9022, elb9023, elb9024])
    ...
    return elb9022, elb9023, elb9024, ecscluster, asgroup, launchconfig, sr, sr_task, conn, conn_task, kr, kr_task
