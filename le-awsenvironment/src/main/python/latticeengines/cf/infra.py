...
APP_APPS = [ 'lpi', 'pls' ]
OAUTH_APPS = [ 'oauth2' ]
ADMINCONSOLE_APPS = [ 'adminconsole', 'matchapi', 'saml',  'api', 'cdl', 'lp' ]
...
def create_public_load_balancers(env, tg_map):
    ...
    resources.append(create_listener_rule(adminconsole_lsnr, tg_map["saml"], "/saml/*"))
...
def provision(environment, stackname):
    ...
    response = client.create_stack(
        ...
        Parameters=[
            ...
            PARAM_PUBLIC_SUBNET_1.config(config.public_subnet_1()),
            PARAM_PUBLIC_SUBNET_2.config(config.public_subnet_2()),
            PARAM_PUBLIC_SUBNET_3.config(config.public_subnet_3()),
