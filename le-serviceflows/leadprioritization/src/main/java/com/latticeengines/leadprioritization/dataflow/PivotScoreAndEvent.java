...
public class PivotScoreAndEvent extends TypesafeDataFlowBuilder<PivotScoreAndEventParameters> {
    ...
    public Node construct(PivotScoreAndEventParameters parameters) {
        ...
        inputTable = inputTable.addFunction(String.format("%s ? 1 : 0", InterfaceName.Event.name()), new FieldList(
                InterfaceName.Event.name()), new FieldMetadata("IsPositiveEvent", Integer.class));
        ...
        aggregatedNode = aggregatedNode.addFunction("TotalEvents == 0 ? 0 : TotalPositiveEvents / TotalEvents",
        ...
        aggregatedNode = aggregatedNode.addFunction(expression, new FieldList("ConversionRate"), new FieldMetadata(
                "Lift", Double.class));
