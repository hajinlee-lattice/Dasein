...
public class AddStandardAttributes extends TypesafeDataFlowBuilder<AddStandardAttributesParameters> {
    ...
    public Node construct(AddStandardAttributesParameters parameters) {
        ...
        Attribute emailOrWebsite = eventTable.getSourceAttribute(SemanticType.Email) != null //
        ? eventTable.getSourceAttribute(SemanticType.Email) //
                : eventTable.getSourceAttribute(SemanticType.Website);

        FieldMetadata fm;

        fm = new FieldMetadata("CompanyName_Entropy", Double.class);
        last = addFunction(last, "std_visidb_ds_companyname_entropy", fm,
                eventTable.getSourceAttribute(SemanticType.CompanyName));

        fm = new FieldMetadata("Title_Length", Integer.class);
        last = addFunction(last, "std_length", fm, //
                eventTable.getSourceAttribute(SemanticType.Title));
        ...
        last = addFunction(last, "std_entropy", fm, //
                eventTable.getSourceAttribute(SemanticType.PhoneNumber));
        ...
        last = addFunction(last, "std_visidb_ds_spamindicator", fm, //
                eventTable.getSourceAttribute(SemanticType.FirstName), //
                eventTable.getSourceAttribute(SemanticType.LastName), //
                eventTable.getSourceAttribute(SemanticType.Title), //
                eventTable.getSourceAttribute(SemanticType.PhoneNumber), //
                eventTable.getSourceAttribute(SemanticType.CompanyName));
        ...
        last = addFunction(last, "std_visidb_ds_industry_group", fm, //
                eventTable.getSourceAttribute(SemanticType.Industry));
