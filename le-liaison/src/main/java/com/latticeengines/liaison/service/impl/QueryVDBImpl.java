...
public class QueryVDBImpl extends Query {
    ...
    private static final Set<String> lasNotImpl = new HashSet<String>(Arrays.asList("Fcn",
            "AndNot", "Meet", "Join", "Pushforward", "Pullback", "Relation", "Copy", "Port",
            "Level", "Conflict", "PivotFunctions"));

    private static final Set<String> laeNotImpl = new HashSet<String>(Arrays.asList("Join", "Meet",
            "Copy", "Relation", "DivideIntoExistingNode", "DivideNewNode", "Port"));
    ...
    private static final Pattern pattern_las_atomic = Pattern
            .compile("^LatticeAddressSetPushforward\\(LatticeAddressExpressionFromLAS\\(LatticeAddressSetMeet\\((.*?)\\)\\), LatticeAddressSetMeet\\((.*?)\\), (LatticeAddressExpressionAtomic\\(.*\\))\\)$");
    private static final Pattern pattern_las_setID_meet = Pattern
            .compile("^LatticeAddressSetPushforward\\(LatticeAddressExpressionFromLAS\\((LatticeAddressSetIdentifier\\(.*?\\))\\), (LatticeAddressSetIdentifier\\(.*?\\)), LatticeAddressExpressionMeet\\((.*)\\)\\)$");
    private static final Pattern pattern_las_setID_atomic = Pattern
            .compile("^LatticeAddressSetPushforward\\(LatticeAddressExpressionFromLAS\\((LatticeAddressSetIdentifier\\(.*?\\))\\), (LatticeAddressSetIdentifier\\(.*?\\)), (LatticeAddressExpressionAtomic\\(.*\\))\\)$");
    private static final Pattern pattern_las_expmeet_meet = Pattern
            .compile("^LatticeAddressSetPushforward\\(LatticeAddressExpressionMeet\\((.*?)\\), LatticeAddressSetMeet\\((.*?)\\), LatticeAddressExpressionMeet\\((.*)\\)\\)$");
    private static final Pattern pattern_las_expmeet_atomic = Pattern
            .compile("^LatticeAddressSetPushforward\\(LatticeAddressExpressionMeet\\((.*?)\\), LatticeAddressSetMeet\\((.*?)\\), (LatticeAddressExpressionAtomic\\(.*\\))\\)$");
    private static final Pattern pattern_las_expatomic_setPI_atomic = Pattern
            .compile("^LatticeAddressSetPushforward\\(LatticeAddressExpressionAtomic\\((.*?)\\), (LatticeAddressSetPi\\(.*?\\)), (LatticeAddressExpressionAtomic\\(.*\\))\\)$");
    private static final Pattern pattern_las_expatomic_setID_meet = Pattern
            .compile("^LatticeAddressSetPushforward\\(LatticeAddressExpressionAtomic\\((LatticeAddressAtomicIdentifier\\(.*?\\))\\), (LatticeAddressSetIdentifier\\(.*?\\)), LatticeAddressExpressionMeet\\((.*)\\)\\)$");
    private static final Pattern pattern_las_expatomic_setID_atomic = Pattern
            .compile("^LatticeAddressSetPushforward\\(LatticeAddressExpressionAtomic\\((LatticeAddressAtomicIdentifier\\(.*?\\))\\), (LatticeAddressSetIdentifier\\(.*?\\)), (LatticeAddressExpressionAtomic\\(.*\\))\\)$");
    ...
    private static final Pattern pattern_entity_lae = Pattern
            .compile("^(LatticeAddressExpression(.*?)\\((.*?)\\))(, LatticeAddressExpression.*|$)");

    private static final Pattern pattern_sqnf = Pattern
            .compile("^(SpecQueryNamedFunction.*?)( SpecQueryNamedFunction.*|$)");
    ...
    public void initFromDefinition(String name, String defn) throws DefinitionException {
        ...
        if (!m_slq.matches()) {
            throw new DefinitionException(String.format(
                    "Maude definition (SpecLatticeQuery) cannot be interpretted: %s", defn));
        }
        ...
        if (!isMatchedTopLevel) {
            Matcher m_las_expatomic_setPI_atomic = pattern_las_expatomic_setPI_atomic
                    .matcher(las_spec);
        ...
        if (!isMatchedTopLevel) {
            Matcher m_las_expatomic_setID_atomic = pattern_las_expatomic_setID_atomic
                    .matcher(las_spec);
        ...
        if (!isMatchedTopLevel) {
            throw new DefinitionException(String.format(
                    "Maude definition (LatticeAddressSet) cannot be interpretted: %s", las_spec));
        }
            ...
            while (Boolean.TRUE) {
                ...
                if (!isMatched) {
                    ...
                    if (m_filter_las.matches()) {
                        if (lasNotImpl.contains(m_filter_las.group(2))) {
                            throw new DefinitionException(String.format(
                                    "Maude definition LatticeAddressSet%s not implemented",
                                    m_filter_las.group(2)));
                        }
                ...
                if (!isMatched) {
                    throw new DefinitionException(
                            String.format(
                                    "Maude definition (LatticeAddressSet) cannot be interpretted: %s",
                                    lasm));
                }
        ...
        if (!lae.equals("empty")) {
            ...
            while (Boolean.TRUE) {
                ...
                if (!isMatched) {
                    ...
                    if (m_entity_lae.matches()) {
                        if (laeNotImpl.contains(m_entity_lae.group(2))) {
                            throw new DefinitionException(String.format(
                                    "Maude definition LatticeAddressExpression%s not implemented",
                                    m_entity_lae.group(2)));
                        }
                ...
                if (!isMatched) {
                    throw new DefinitionException(
                            String.format(
                                    "Maude definition (LatticeAddressExpression) cannot be interpretted: %s",
                                    lae));
                }
        ...
        while (Boolean.TRUE) {
            ...
            if (!m_sqnf.matches()) {
                throw new DefinitionException(String.format(
                        "Maude definition (SpecQueryNamedFunction) cannot be interpretted: %s",
                        sqnf));
            }
