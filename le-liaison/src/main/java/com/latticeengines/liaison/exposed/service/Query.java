...
public abstract class Query {
    ...
    public Query(String name, List<QueryColumn> columns, List<String> filterDefns,
            List<String> entityDefns, String resultSetDefn) {
        initFromValues(name, columns, filterDefns, entityDefns, resultSetDefn);
    }

    public Query(Query other) {
        initFromValues(other.name, other.columns, other.filterDefns, other.entityDefns,
                other.resultSetDefn);
    }
    ...
    public QueryColumn getColumn(String colname) throws RuntimeException {
        if (!columnNames.contains(colname)) {
            throw new RuntimeException(String.format(
                    "Column \"%s\" does not exist in query \"%s\"", colname, getName()));
        }
    ...
    public void updateColumn(QueryColumn column) {
        if (columnNames.contains(column.getName())) {
            ...
            columns.get(idx).initFromValues(column.getName(), column.getExpression(),
                    column.getMetadata());
        } else {
            appendColumn(column);
        }
    ...
    public void initFromValues(String name, List<QueryColumn> columns, List<String> filterDefns,
            List<String> entityDefns, String resultSetDefn) {
        this.name = name;
