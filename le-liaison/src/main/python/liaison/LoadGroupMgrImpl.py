...
class LoadGroupMgrImpl( LoadGroupMgr ):
    ...
    def getConfig(self, appName=None):

        g_old = self._config.find(".//groups")
        g_new = etree.Element( 'groups' )
        for name, g in sorted(self._groups.iteritems()):
            g_new.append( g )
        self._config.replace( g_old, g_new )
        if appName is not None:
            self._config.set( 'appName', appName )
        return etree.tostring(self._config,pretty_print=True)


    def commit( self ):

        self._conn_mgr.installDLConfigFile( self.getConfig() )
