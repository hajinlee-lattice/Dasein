...
## q = QueryVDBImpl.InitFromDefn( defn )
    ...
    def initFromDefn(cls, name, defn):
        ...
        if not isMatchedTopLevel:
            s3 = re.search( '^LatticeAddressSetPushforward\(LatticeAddressExpressionAtomic\((.*?)\), (LatticeAddressSetPi\(.*?\)), (LatticeAddressExpressionAtomic\(.*\))\)$', las_spec )
            if s3:
                lasm = '({0})'.format( s3.group(2) )
                lae = '({0})'.format( s3.group(3) )
                isMatchedTopLevel = True
        ...
        if not isMatchedTopLevel:
            s3 = re.search( '^LatticeAddressSetPushforward\(LatticeAddressExpressionAtomic\((LatticeAddressAtomicIdentifier\(.*?\))\), (LatticeAddressSetIdentifier\(.*?\)), (LatticeAddressExpressionAtomic\(.*\))\)$', las_spec )
            if s3:
                lasm = '({0})'.format( s3.group(2) )
                lae = '({0})'.format( s3.group(3) )
                isMatchedTopLevel = True
